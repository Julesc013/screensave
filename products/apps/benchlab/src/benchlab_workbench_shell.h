...
typedef struct benchlab_workbench_plasma_instrument_inspect_tag {
    ...
    const char *visualintent_source_path;
    unsigned long visualintent_candidate_count;
    const char *selected_candidate_id;
    const char *candidate_product;
    const char *requested_spec;
    const char *migrated_spec_summary;
    const char *visualintent_spec_summary;
    const char *resolved_plan;
    const char *resolved_plan_summary;
    const char *visualintent_plan_summary;
    const char *degradation_reason;
    const char *field_pipeline;
    const char *material_response;
    const char *treatment_stack;
    const char *software_reference_path;
    const char *gl11_optionality;
    const char *capture_refs;
    const char *packc_status;
    const char *visualintent_proof_refs;
