...
const screensave_preset_descriptor g_gallery_presets[] = {
    {
...
static const gallery_preset_values g_gallery_preset_values[] = {
    { GALLERY_SCENE_COMPATIBILITY, GALLERY_MOTION_CALM },
    { GALLERY_SCENE_COMPATIBILITY, GALLERY_MOTION_STANDARD },
...
{
    if (count_out != NULL) {
        *count_out = (unsigned int)(sizeof(g_gallery_presets) / sizeof(g_gallery_presets[0]));
    }

    return g_gallery_presets;
}
...
const gallery_preset_values *gallery_find_preset_values(const char *preset_key)
{
    ...
    for (index = 0U; index < preset_count; ++index) {
        if (presets[index].preset_key != NULL && strcmp(presets[index].preset_key, preset_key) == 0) {
            return &g_gallery_preset_values[index];
        }
    }
...
)
{
    ...
    const gallery_preset_values *preset_values;
    ...
    preset_values = gallery_find_preset_values(preset_key);
