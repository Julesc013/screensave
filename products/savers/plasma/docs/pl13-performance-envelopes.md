...
| --- | --- | --- | --- | --- | --- |
| `classic_gdi` | Validated | [`validation/captures/pl13/benchlab-plasma-gdi.txt`](../../../../validation/captures/pl13/benchlab-plasma-gdi.txt) | Classic session creation and report-mode startup succeeded on forced `gdi`. | Runtime surface remains classic raster-plus-flat with no richer-lane uplift. | Repeated lower-band create/destroy plus stepping is covered in smoke. |
| `classic_gl11` | Validated | [`validation/captures/pl13/benchlab-plasma-gl11.txt`](../../../../validation/captures/pl13/benchlab-plasma-gl11.txt) | Classic session creation and report-mode startup succeeded on forced `gl11`. | Compat lane remains the stable accelerated baseline; shared bounded transition subset is also routable here. | Repeated lower-band create/destroy plus stepping is covered in smoke. |
...
- The default classic path on `gdi` and `gl11` remains the most strongly exercised stability surface.
