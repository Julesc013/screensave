...
The stable runtime cut itself does not broaden.
The stable widening in this recut is narrow and support-surface-only:

- BenchLab reporting remains part of the stable support surface
- PX40 read-only authoring and ops reports now belong to the stable support surface:
  - `validate`
  - `authoring-report`
...
- PX41 stable local metadata and report surfaces now belong to the stable support surface when they only restate stable product truth:
  ...
  - the stable `dark_room_support` control profile
  - the `lava_remix_builtin` provenance entry
  - `integration-report`, `control-report`, and `curation-report` over those stable surfaces

## What Is Available As Experimental

The following remain implemented but explicitly experimental:
...
The stable release posture remains centered on:
...
They are not the hidden minimum.

## Major Caveats And Known Limits
...
- numeric performance SLAs are still not claimed

## Release Tone Boundary

This draft is intentionally factual.
It does not claim broad stable widening, broad integration maturity, or broad cross-hardware certainty that the repo does not prove.
