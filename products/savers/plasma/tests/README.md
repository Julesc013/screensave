...
U06 adds the first explicit retuning checkpoint by re-running the XP-compatible
build and smoke path, refreshing the deterministic preset and palette crowding
audit, capturing the stable default path on `gdi` and `gl11`, capturing the
retuned stable classic journey path, and recording the deterministic classic and
dark-room support control profiles that now anchor preview and long-run review.

U07 adds a real proof-harness layer on top of the earlier smoke and BenchLab substrate: deterministic render-signature and pixel-difference checks for the shipped settings subset, an exhaustive stable preset/theme/lane matrix, a bounded experimental coverage report, a shared proof-result taxonomy, and a stronger preset-signature and palette audit for duplicate or near-duplicate detection.

U08 records the formal QA bar, the surface-by-surface evaluation, the explicit
acceptance decisions, and the narrow metadata cleanup that keeps stable support
profiles and stable curated collections from implying transition stability they
have not yet proved.

U09 adds the final corrective recut layer: explicit stable and
experimental/deferred classification, the final ship-posture decision, and a
recut-proof note that binds the living cut to the current U07/U08 evidence plus
fresh consistency and report reruns.
