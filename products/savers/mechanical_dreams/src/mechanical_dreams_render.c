...
static const int g_mechanical_dreams_unit_x[16] = {
    1024, 946, 724, 392, 0, -392, -724, -946,
    -1024, -946, -724, -392, 0, 392, 724, 946
};

static const int g_mechanical_dreams_unit_y[16] = {
    0, 392, 724, 946, 1024, 946, 724, 392,
    0, -392, -724, -946, -1024, -946, -724, -392
};

static screensave_color mechanical_dreams_background_color(const screensave_saver_session *session)
{
...
static screensave_color mechanical_dreams_mix_color(
...
static void mechanical_dreams_draw_fill(
...
static void mechanical_dreams_draw_frame(
    screensave_renderer *renderer,
    int x,
    int y,
    int width,
...
static void mechanical_dreams_draw_gear(
    screensave_renderer *renderer,
    const mechanical_dreams_gear *gear,
...
)
{
    ...
    for (point_index = 0U; point_index < 16U; ++point_index) {
        ...
        points[point_index].x = gear->x + (g_mechanical_dreams_unit_x[vector_index] * local_radius) / 1024;
        points[point_index].y = gear->y + (g_mechanical_dreams_unit_y[vector_index] * local_radius) / 1024;
    }
    points[16] = points[0];
    screensave_renderer_draw_polyline(renderer, points, 17U, outline_color);
    mechanical_dreams_draw_fill(renderer, gear->x - 2, gear->y - 2, 5, 5, hub_color);
}

static void mechanical_dreams_draw_pointer(
    screensave_renderer *renderer,
    int center_x,
    int center_y,
    int radius,
...
)
{
    ...
    end_point.x = center_x + (g_mechanical_dreams_unit_x[vector_index] * pointer_radius) / 1024;
    end_point.y = center_y + (g_mechanical_dreams_unit_y[vector_index] * pointer_radius) / 1024;
    screensave_renderer_draw_line(renderer, &start_point, &end_point, color);
}

static void mechanical_dreams_draw_gear_train(
...
)
{
    ...
    rail_color = mechanical_dreams_mix_color(mechanical_dreams_background_color(session), session->theme->accent_color, 120U);

    for (index = 1U; index < MECHANICAL_DREAMS_MAX_GEARS; ++index) {
    ...
    for (index = 0U; index < MECHANICAL_DREAMS_MAX_GEARS; ++index) {
        mechanical_dreams_draw_gear(renderer, &session->gears[index], outline_color, hub_color);
    }

    if (session->event_pulse > 0U) {
        mechanical_dreams_draw_fill(
            renderer,
            12,
...
static void mechanical_dreams_draw_cam_bank(
    screensave_saver_session *session,
    screensave_renderer *renderer
)
{
    ...
    follower_color = mechanical_dreams_mix_color(mechanical_dreams_background_color(session), accent_color, 136U);
    ...
    for (index = 0U; index < MECHANICAL_DREAMS_MAX_GEARS; ++index) {
        ...
        mechanical_dreams_draw_gear(renderer, &session->gears[index], outline_color, accent_color);
    }
}

static void mechanical_dreams_draw_dial_assembly(
    screensave_saver_session *session,
    screensave_renderer *renderer
)
{
    ...
    for (index = 0U; index < MECHANICAL_DREAMS_MAX_DIALS; ++index) {
        ...
        mechanical_dreams_draw_pointer(
            renderer,
    ...
    for (index = 0U; index < MECHANICAL_DREAMS_MAX_GEARS; ++index) {
        mechanical_dreams_draw_gear(renderer, &session->gears[index], frame_color, pointer_color);
    }
}
...
)
{
    ...
    screensave_renderer_clear(environment->renderer, mechanical_dreams_background_color(session));

    switch (session->config.scene_mode) {
    case MECHANICAL_DREAMS_SCENE_CAM_BANK:
        mechanical_dreams_draw_cam_bank(session, environment->renderer);
        break;

    case MECHANICAL_DREAMS_SCENE_DIAL_ASSEMBLY:
        mechanical_dreams_draw_dial_assembly(session, environment->renderer);
        break;

    case MECHANICAL_DREAMS_SCENE_GEAR_TRAIN:
    default:
        mechanical_dreams_draw_gear_train(session, environment->renderer);
        break;
    }

    if (!session->preview_mode) {
        frame_color = mechanical_dreams_mix_color(mechanical_dreams_background_color(session), session->theme->accent_color, 120U);
        outer_frame.x = 8;
        outer_frame.y = 8;
