#include <stdlib.h>

#include "mechanical_dreams_internal.h"

static const screensave_theme_descriptor *mechanical_dreams_resolve_theme(
    const screensave_config_binding *binding
)
{
    if (binding != NULL && binding->common_config != NULL) {
        return mechanical_dreams_find_theme_descriptor(binding->common_config->theme_key);
    }

    return mechanical_dreams_find_theme_descriptor(MECHANICAL_DREAMS_DEFAULT_THEME_KEY);
}

static const mechanical_dreams_config *mechanical_dreams_resolve_config(
    const screensave_config_binding *binding
)
{
    if (
        binding == NULL ||
        binding->product_config == NULL ||
        binding->product_config_size != sizeof(mechanical_dreams_config)
    ) {
        return NULL;
    }

    return (const mechanical_dreams_config *)binding->product_config;
}

static unsigned long mechanical_dreams_step_interval(const screensave_saver_session *session)
{
    if (session == NULL) {
        return 70UL;
    }

    switch (session->config.speed_mode) {
    case MECHANICAL_DREAMS_SPEED_PATIENT:
        return 95UL;

    case MECHANICAL_DREAMS_SPEED_BRISK:
        return 36UL;

    case MECHANICAL_DREAMS_SPEED_STANDARD:
    default:
        return 62UL;
    }
}

static unsigned int mechanical_dreams_density_count(
    int density_mode,
    unsigned int sparse_value,
    unsigned int standard_value,
    unsigned int dense_value
)
{
    switch (density_mode) {
    case MECHANICAL_DREAMS_DENSITY_SPARSE:
        return sparse_value;

    case MECHANICAL_DREAMS_DENSITY_DENSE:
        return dense_value;

    case MECHANICAL_DREAMS_DENSITY_STANDARD:
    default:
        return standard_value;
    }
}

static int mechanical_dreams_min_int(int a, int b)
{
    return a < b ? a : b;
}

static void mechanical_dreams_clear_layout(screensave_saver_session *session)
{
    unsigned int index;

    if (session == NULL) {
        return;
    }

    for (index = 0U; index < MECHANICAL_DREAMS_MAX_GEARS; ++index) {
        session->gears[index].active = 0;
        session->gears[index].phase = 0U;
    }
    for (index = 0U; index < MECHANICAL_DREAMS_MAX_DIALS; ++index) {
        session->dials[index].active = 0;
        session->dials[index].angle = 0U;
        session->dials[index].target_angle = 0U;
    }
    for (index = 0U; index < MECHANICAL_DREAMS_MAX_FOLLOWERS; ++index) {
        session->follower_heights[index] = 0;
        session->follower_targets[index] = 0;
    }
}

static void mechanical_dreams_build_gear_train(screensave_saver_session *session)
{
    unsigned int count;
    unsigned int index;
    int spacing;
    int baseline;
    int radius;

    if (session == NULL) {
        return;
    }

    count = mechanical_dreams_density_count(session->config.density_mode, 3U, 4U, 5U);
    if (session->preview_mode && count > 3U) {
        count = 3U;
    }

    radius = mechanical_dreams_min_int(session->drawable_size.width / (int)(count * 3U), session->drawable_size.height / 5);
    if (radius < 14) {
        radius = 14;
    }

    spacing = (session->drawable_size.width - (radius * 2)) / (int)(count + 1U);
    baseline = session->drawable_size.height / 2;

    for (index = 0U; index < count; ++index) {
        session->gears[index].active = 1;
        session->gears[index].x = spacing * (int)(index + 1U);
        session->gears[index].y = baseline + ((index & 1U) ? radius / 3 : -(radius / 3));
        session->gears[index].radius = radius - ((int)index % 2);
        session->gears[index].direction = (index & 1U) ? -1 : 1;
        session->gears[index].phase = (unsigned int)mechanical_dreams_rng_range(&session->rng, 256UL);
    }
}

static void mechanical_dreams_build_cam_bank(screensave_saver_session *session)
{
    unsigned int count;
    unsigned int index;
    int spacing;
    int y;
    int radius;

    if (session == NULL) {
        return;
    }

    count = mechanical_dreams_density_count(session->config.density_mode, 3U, 4U, 5U);
    if (session->preview_mode && count > 3U) {
        count = 3U;
    }

    radius = mechanical_dreams_min_int(session->drawable_size.width / (int)(count * 4U), session->drawable_size.height / 6);
    if (radius < 12) {
        radius = 12;
    }
    spacing = session->drawable_size.width / (int)(count + 1U);
    y = (session->drawable_size.height * 2) / 3;

    for (index = 0U; index < count; ++index) {
        session->gears[index].active = 1;
        session->gears[index].x = spacing * (int)(index + 1U);
        session->gears[index].y = y;
        session->gears[index].radius = radius;
        session->gears[index].direction = 1;
        session->gears[index].phase = (unsigned int)mechanical_dreams_rng_range(&session->rng, 256UL);
        session->follower_heights[index] = radius;
        session->follower_targets[index] = radius;
    }
}

static void mechanical_dreams_build_dial_assembly(screensave_saver_session *session)
{
    unsigned int count;
    unsigned int index;
    int columns;
    int rows;
    int spacing_x;
    int spacing_y;
    int radius;

    if (session == NULL) {
        return;
    }

    count = mechanical_dreams_density_count(session->config.density_mode, 2U, 3U, 4U);
    if (session->preview_mode && count > 2U) {
        count = 2U;
    }

    columns = 2;
    rows = 2;
    radius = mechanical_dreams_min_int(session->drawable_size.width / 7, session->drawable_size.height / 5);
    if (radius < 16) {
        radius = 16;
    }
    spacing_x = session->drawable_size.width / 3;
    spacing_y = session->drawable_size.height / 3;

    for (index = 0U; index < count; ++index) {
        session->dials[index].active = 1;
        session->dials[index].x = spacing_x * (int)((index % columns) + 1U);
        session->dials[index].y = spacing_y * (int)((index / columns) + 1U);
        session->dials[index].radius = radius;
        session->dials[index].angle = (unsigned int)mechanical_dreams_rng_range(&session->rng, 256UL);
        session->dials[index].target_angle = (unsigned int)mechanical_dreams_rng_range(&session->rng, 256UL);
    }

    for (index = 0U; index < count; ++index) {
        session->gears[index].active = 1;
        session->gears[index].x = spacing_x * (int)(index + 1U) - spacing_x / 2;
        session->gears[index].y = session->drawable_size.height - radius * 2;
        session->gears[index].radius = radius / 2;
        session->gears[index].direction = (index & 1U) ? -1 : 1;
        session->gears[index].phase = (unsigned int)mechanical_dreams_rng_range(&session->rng, 256UL);
    }
}

static void mechanical_dreams_configure_layout(screensave_saver_session *session)
{
    if (session == NULL) {
        return;
    }

    mechanical_dreams_clear_layout(session);
    switch (session->config.scene_mode) {
    case MECHANICAL_DREAMS_SCENE_CAM_BANK:
        mechanical_dreams_build_cam_bank(session);
        break;

    case MECHANICAL_DREAMS_SCENE_DIAL_ASSEMBLY:
        mechanical_dreams_build_dial_assembly(session);
        break;

    case MECHANICAL_DREAMS_SCENE_GEAR_TRAIN:
    default:
        mechanical_dreams_build_gear_train(session);
        break;
    }
}

static void mechanical_dreams_initialize_session(
    screensave_saver_session *session,
    const mechanical_dreams_config *config,
    const screensave_saver_environment *environment
)
{
    unsigned long seed;

    if (session == NULL || config == NULL || environment == NULL) {
        return;
    }

    session->config = *config;
    session->theme = mechanical_dreams_resolve_theme(environment->config_binding);
    session->drawable_size = environment->drawable_size;
    session->preview_mode = environment->mode == SCREENSAVE_SESSION_MODE_PREVIEW;
    seed = environment->seed.stream_seed != 0UL ? environment->seed.stream_seed : 0x24681357UL;
    mechanical_dreams_rng_seed(&session->rng, seed);
    session->step_accumulator = 0UL;
    session->event_accumulator = 0UL;
    session->master_phase = 0U;
    session->event_pulse = 0U;
    session->counter_window = 0U;
    mechanical_dreams_configure_layout(session);
}

static unsigned int mechanical_dreams_triangle_wave(unsigned int phase)
{
    unsigned int local_phase;

    local_phase = phase & 255U;
    if (local_phase < 128U) {
        return local_phase * 2U;
    }

    return (255U - local_phase) * 2U;
}

static void mechanical_dreams_update_gears(screensave_saver_session *session)
{
    unsigned int index;
    unsigned int phase_step;

    if (session == NULL) {
        return;
    }

    phase_step = (unsigned int)(session->config.speed_mode + 1);
    for (index = 0U; index < MECHANICAL_DREAMS_MAX_GEARS; ++index) {
        if (!session->gears[index].active) {
            continue;
        }

        if (session->gears[index].direction >= 0) {
            session->gears[index].phase = (session->gears[index].phase + phase_step + index) & 255U;
        } else {
            session->gears[index].phase = (session->gears[index].phase - (phase_step + index)) & 255U;
        }
    }
}

static void mechanical_dreams_update_cam_followers(screensave_saver_session *session)
{
    unsigned int index;

    if (session == NULL) {
        return;
    }

    for (index = 0U; index < MECHANICAL_DREAMS_MAX_FOLLOWERS; ++index) {
        if (!session->gears[index].active) {
            continue;
        }

        session->follower_targets[index] = (int)(mechanical_dreams_triangle_wave(session->gears[index].phase) / 12U);
        if (session->follower_heights[index] < session->follower_targets[index]) {
            session->follower_heights[index] += 1;
        } else if (session->follower_heights[index] > session->follower_targets[index]) {
            session->follower_heights[index] -= 1;
        }
    }
}

static void mechanical_dreams_update_dials(screensave_saver_session *session)
{
    unsigned int index;
    int delta;

    if (session == NULL) {
        return;
    }

    for (index = 0U; index < MECHANICAL_DREAMS_MAX_DIALS; ++index) {
        if (!session->dials[index].active) {
            continue;
        }

        delta = (int)session->dials[index].target_angle - (int)session->dials[index].angle;
        if (delta > 128) {
            delta -= 256;
        } else if (delta < -128) {
            delta += 256;
        }

        if (delta > 0) {
            session->dials[index].angle = (session->dials[index].angle + 1U + (unsigned int)(delta > 12)) & 255U;
        } else if (delta < 0) {
            session->dials[index].angle = (session->dials[index].angle - (1U + (unsigned int)(delta < -12))) & 255U;
        } else {
            session->dials[index].target_angle = (unsigned int)mechanical_dreams_rng_range(&session->rng, 256UL);
        }
    }
}

static void mechanical_dreams_run_step(screensave_saver_session *session)
{
    if (session == NULL) {
        return;
    }

    session->master_phase = (session->master_phase + (unsigned int)(session->config.speed_mode + 1)) & 255U;
    session->counter_window = (session->counter_window + 1U) % 1000U;
    mechanical_dreams_update_gears(session);
    if (session->config.scene_mode == MECHANICAL_DREAMS_SCENE_CAM_BANK) {
        mechanical_dreams_update_cam_followers(session);
    }
    if (session->config.scene_mode == MECHANICAL_DREAMS_SCENE_DIAL_ASSEMBLY) {
        mechanical_dreams_update_dials(session);
    }

    if (session->event_pulse > 0U) {
        session->event_pulse -= 1U;
    }
}

void mechanical_dreams_rng_seed(mechanical_dreams_rng_state *state, unsigned long seed)
{
    if (state == NULL) {
        return;
    }

    state->state = seed != 0UL ? seed : 0x9151C0DEUL;
}

unsigned long mechanical_dreams_rng_next(mechanical_dreams_rng_state *state)
{
    if (state == NULL) {
        return 0UL;
    }

    state->state = state->state * 1664525UL + 1013904223UL;
    return state->state;
}

unsigned long mechanical_dreams_rng_range(mechanical_dreams_rng_state *state, unsigned long upper_bound)
{
    if (upper_bound == 0UL) {
        return 0UL;
    }

    return mechanical_dreams_rng_next(state) % upper_bound;
}

int mechanical_dreams_create_session(
    const screensave_saver_module *module,
    screensave_saver_session **session_out,
    const screensave_saver_environment *environment
)
{
    screensave_saver_session *session;
    mechanical_dreams_config config;
    const mechanical_dreams_config *bound_config;
    screensave_common_config safe_common;
    mechanical_dreams_config safe_config;

    (void)module;

    if (session_out == NULL || environment == NULL) {
        return 0;
    }

    mechanical_dreams_config_set_defaults(&safe_common, &config, sizeof(config));
    bound_config = mechanical_dreams_resolve_config(environment->config_binding);
    if (bound_config != NULL) {
        config = *bound_config;
    }

    if (environment->config_binding != NULL && environment->config_binding->common_config != NULL) {
        safe_common = *environment->config_binding->common_config;
    } else {
        screensave_common_config_set_defaults(&safe_common);
    }
    safe_config = config;
    mechanical_dreams_config_clamp(&safe_common, &safe_config, sizeof(safe_config));

    session = (screensave_saver_session *)calloc(1U, sizeof(*session));
    if (session == NULL) {
        return 0;
    }

    mechanical_dreams_initialize_session(session, &safe_config, environment);
    *session_out = session;
    return 1;
}

void mechanical_dreams_destroy_session(screensave_saver_session *session)
{
    if (session == NULL) {
        return;
    }

    free(session);
}

void mechanical_dreams_resize_session(
    screensave_saver_session *session,
    const screensave_saver_environment *environment
)
{
    if (session == NULL || environment == NULL) {
        return;
    }

    session->drawable_size = environment->drawable_size;
    mechanical_dreams_configure_layout(session);
}

void mechanical_dreams_step_session(
    screensave_saver_session *session,
    const screensave_saver_environment *environment
)
{
    unsigned long interval;

    if (session == NULL || environment == NULL) {
        return;
    }

    session->step_accumulator += environment->clock.delta_millis;
    session->event_accumulator += environment->clock.delta_millis;
    interval = mechanical_dreams_step_interval(session);

    while (session->step_accumulator >= interval) {
        session->step_accumulator -= interval;
        mechanical_dreams_run_step(session);
    }

    if (session->event_accumulator >= 2200UL) {
        session->event_accumulator = 0UL;
        session->event_pulse = 18U;
        if (session->config.scene_mode == MECHANICAL_DREAMS_SCENE_DIAL_ASSEMBLY) {
            unsigned int index;

            for (index = 0U; index < MECHANICAL_DREAMS_MAX_DIALS; ++index) {
                if (session->dials[index].active) {
                    session->dials[index].target_angle = (unsigned int)mechanical_dreams_rng_range(&session->rng, 256UL);
                }
            }
        }
    }
}
