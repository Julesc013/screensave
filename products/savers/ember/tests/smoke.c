...
int main(void)
{
    ...
    module = ember_get_module();
    if (!screensave_saver_module_is_valid(module)) {
        return 1;
    }

    ember_config_set_defaults(&common_config, &product_config, sizeof(product_config));
    ember_config_clamp(&common_config, &product_config, sizeof(product_config));
    if (!screensave_common_config_validate(&common_config, &issue_flags)) {
        return 2;
    }

    if (ember_find_preset_values("ember_lava") == NULL) {
        return 3;
    }

    if (ember_find_theme_descriptor("aurora_cool") == NULL) {
        return 4;
    }
    ...
    session = NULL;
    if (!ember_create_session(module, &session, &environment) || session == NULL) {
        return 5;
    }

    environment.clock.delta_millis = 33UL;
    ember_step_session(session, &environment);
