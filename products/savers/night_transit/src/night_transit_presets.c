...
const screensave_preset_descriptor g_night_transit_presets[] = {
    {
        "wet_motorway",
        "Wet Motorway",
...
static const night_transit_preset_values g_night_transit_preset_values[] = {
    { NIGHT_TRANSIT_SCENE_MOTORWAY, NIGHT_TRANSIT_SPEED_CRUISE, NIGHT_TRANSIT_LIGHTS_STANDARD },
    { NIGHT_TRANSIT_SCENE_RAIL, NIGHT_TRANSIT_SPEED_GLIDE, NIGHT_TRANSIT_LIGHTS_STANDARD },
    { NIGHT_TRANSIT_SCENE_MOTORWAY, NIGHT_TRANSIT_SPEED_EXPRESS, NIGHT_TRANSIT_LIGHTS_DENSE },
    { NIGHT_TRANSIT_SCENE_HARBOR, NIGHT_TRANSIT_SPEED_GLIDE, NIGHT_TRANSIT_LIGHTS_STANDARD },
    { NIGHT_TRANSIT_SCENE_RAIL, NIGHT_TRANSIT_SPEED_GLIDE, NIGHT_TRANSIT_LIGHTS_SPARSE }
};

const screensave_preset_descriptor *night_transit_get_presets(unsigned int *count_out)
{
    if (count_out != NULL) {
        *count_out = (unsigned int)(sizeof(g_night_transit_presets) / sizeof(g_night_transit_presets[0]));
    }

    return g_night_transit_presets;
}
...
const night_transit_preset_values *night_transit_find_preset_values(const char *preset_key)
{
    ...
    for (index = 0U; index < preset_count; ++index) {
        if (presets[index].preset_key != NULL && strcmp(presets[index].preset_key, preset_key) == 0) {
            return &g_night_transit_preset_values[index];
        }
    }
...
)
{
    ...
    const night_transit_preset_values *preset_values;
    ...
    preset_values = night_transit_find_preset_values(preset_key);
