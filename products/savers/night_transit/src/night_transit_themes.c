...
const screensave_theme_descriptor g_night_transit_themes[] = {
    {
        "wet_motorway",
        "Wet Motorway",
...
{
    if (count_out != NULL) {
        *count_out = (unsigned int)(sizeof(g_night_transit_themes) / sizeof(g_night_transit_themes[0]));
    }

    return g_night_transit_themes;
}
...
const screensave_theme_descriptor *night_transit_find_theme_descriptor(const char *theme_key)
{
    ...
    themes = night_transit_get_themes(&theme_count);
