...
static screensave_color city_mix_color(
...
static screensave_color city_background_color(const screensave_saver_session *session)
{
...
static int city_scene_horizon_y(const screensave_saver_session *session)
{
    int height;
    int horizon;
...
static void city_draw_polyline(
    screensave_renderer *renderer,
...
)
{
    ...
    upper_color = city_mix_color(base_color, session->theme->primary_color, 28U);
    mid_color = city_mix_color(base_color, session->theme->primary_color, 48U);
    low_color = city_mix_color(base_color, session->theme->accent_color, 56U);
    band_top = horizon_y / 2;
    band_mid = horizon_y / 3;
    band_low = horizon_y / 6;

    city_draw_fill(renderer, 0, 0, session->drawable_size.width, band_top, upper_color);
    city_draw_fill(renderer, 0, band_top, session->drawable_size.width, band_mid, mid_color);
    city_draw_fill(renderer, 0, horizon_y - band_low, session->drawable_size.width, band_low, low_color);
}

static void city_draw_fog(
    const screensave_saver_session *session,
    screensave_renderer *renderer,
    screensave_color base_color
)
{
    ...
    horizon_y = city_scene_horizon_y(session);
    width = session->drawable_size.width;
    for (index = 0U; index < session->fog_count; ++index) {
        ...
        fog_color = city_mix_color(base_color, session->theme->accent_color, 32U + (unsigned int)session->fog_bands[index].opacity);
        ...
        city_draw_fill(renderer, x, y, width, height, fog_color);
    }
}

static void city_draw_windows(
    const screensave_saver_session *session,
    screensave_renderer *renderer,
    ...
    for (row = 0; row < structure->window_rows; ++row) {
        for (column = 0; column < structure->window_columns; ++column) {
            ...
            if ((gate % 100U) < (unsigned int)light_bias) {
                color = city_mix_color(window_off_color, window_on_color, (unsigned int)(120 + (light_bias % 96)));
            } else {
                color = window_off_color;
            }
            city_draw_fill(renderer, window_x, window_y, cell_width - 1, cell_height - 1, color);
        }
    }
}

static void city_draw_structure(
    const screensave_saver_session *session,
    screensave_renderer *renderer,
    const city_structure *structure,
    int horizon_y,
...
)
{
    ...
    block_color = city_mix_color(base_color, accent_color, 40U + (unsigned int)(structure->depth * 18));
    if (session->config.scene_mode == CITY_SCENE_HARBOR) {
        block_color = city_mix_color(block_color, session->theme->primary_color, 32U);
    } else if (session->config.scene_mode == CITY_SCENE_WINDOW_RIVER) {
        block_color = city_mix_color(block_color, session->theme->accent_color, 48U);
    }
    edge_color = city_mix_color(base_color, session->theme->accent_color, 96U + (unsigned int)(structure->depth * 16));
    window_on_color = city_mix_color(session->theme->accent_color, edge_color, 168U);
    window_off_color = city_mix_color(base_color, edge_color, 54U);

    city_draw_fill(renderer, x, y, width, height, block_color);
    outline.x = x;
    outline.y = y;
    outline.width = width;
    ...
    if (structure->roof_style == 0) {
        ...
        city_draw_polyline(renderer, roof_points, 3U, edge_color);
        ...
        city_draw_polyline(renderer, roof_points, 5U, edge_color);
    } else {
        city_draw_fill(renderer, x + 2, y - 1, width - 4, 2, edge_color);
    }
    ...
    if (session->config.scene_mode == CITY_SCENE_HARBOR) {
        ...
        crane_points[3].y = y;
        city_draw_polyline(renderer, crane_points, 4U, edge_color);
    }
}

static void city_draw_lights(
    const screensave_saver_session *session,
    screensave_renderer *renderer,
    screensave_color base_color,
    int horizon_y,
...
)
{
    ...
    light_color = city_mix_color(base_color, session->theme->accent_color, 192U);
    for (index = 0U; index < session->light_count; ++index) {
        ...
        light_color = city_mix_color(base_color, session->theme->accent_color, (unsigned int)brightness);
        city_draw_fill(renderer, x, y, width, height, light_color);
        if (session->config.scene_mode != CITY_SCENE_SKYLINE) {
            screensave_color reflection_color;

            reflection_color = city_mix_color(base_color, session->theme->primary_color, (unsigned int)(96 + phase / 2));
            city_draw_fill(renderer, x - 1, horizon_y + 2, width + 2, 4 + (phase % 10), reflection_color);
        }
    }
}

static void city_draw_ground(
    const screensave_saver_session *session,
    screensave_renderer *renderer,
    screensave_color base_color,
    int horizon_y
)
{
    ...
    ground_color = city_mix_color(base_color, session->theme->primary_color, 22U);
    rail_color = city_mix_color(base_color, session->theme->accent_color, 72U);
    height = session->drawable_size.height - horizon_y;
    if (height < 10) {
        height = 10;
    }
    city_draw_fill(renderer, 0, horizon_y, session->drawable_size.width, height, ground_color);
    rail_rect.x = 0;
...
static void city_draw_reflections(
    const screensave_saver_session *session,
    screensave_renderer *renderer,
    int horizon_y
)
{
    ...
    for (index = 0U; index < session->structure_count; ++index) {
        ...
        color = city_mix_color(session->theme->primary_color, session->theme->accent_color, 128U);
        reflection_rect.x = x;
...
)
{
    ...
    horizon_y = city_scene_horizon_y(session);
    scene_span = city_scene_span(session);
    width = session->drawable_size.width;

    city_draw_sky(session, renderer, base_color, horizon_y);
    city_draw_fog(session, renderer, base_color);
    city_draw_ground(session, renderer, base_color, horizon_y);
    city_draw_reflections(session, renderer, horizon_y);
    ...
    city_draw_lights(session, renderer, base_color, horizon_y, scene_span);

    if (session->config.scene_mode == CITY_SCENE_WINDOW_RIVER) {
        screensave_color river_color;

        river_color = city_mix_color(base_color, session->theme->accent_color, 120U);
        city_draw_fill(renderer, 0, horizon_y - 6, width, 2, river_color);
        city_draw_fill(renderer, 0, horizon_y + 10, width, 1, river_color);
    }
}
...
)
{
    ...
    base_color = city_background_color(session);
    screensave_renderer_clear(renderer, base_color);
    city_draw_scene(session, renderer, base_color);
}
