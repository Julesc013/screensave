...
const screensave_theme_descriptor g_fractal_atlas_themes[] = {
    {
        "atlas_monochrome",
        "Atlas Monochrome",
...
{
    if (count_out != NULL) {
        *count_out = (unsigned int)(sizeof(g_fractal_atlas_themes) / sizeof(g_fractal_atlas_themes[0]));
    }

    return g_fractal_atlas_themes;
}
...
const screensave_theme_descriptor *fractal_atlas_find_theme_descriptor(const char *theme_key)
{
    ...
    themes = fractal_atlas_get_themes(&theme_count);
