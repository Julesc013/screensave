...
static const fractal_atlas_waypoint g_fractal_atlas_voyage_points[FRACTAL_ATLAS_ROUTE_POINT_COUNT] = {
...
static const fractal_atlas_waypoint g_fractal_atlas_atlas_points[FRACTAL_ATLAS_ROUTE_POINT_COUNT] = {
...
static const fractal_atlas_waypoint g_fractal_atlas_julia_points[FRACTAL_ATLAS_ROUTE_POINT_COUNT] = {
...
static const screensave_theme_descriptor *fractal_atlas_resolve_theme(const screensave_config_binding *binding)
{
    if (binding != NULL && binding->common_config != NULL) {
        return fractal_atlas_find_theme_descriptor(binding->common_config->theme_key);
    }

    return fractal_atlas_find_theme_descriptor(FRACTAL_ATLAS_DEFAULT_THEME_KEY);
}

static const fractal_atlas_config *fractal_atlas_resolve_config(const screensave_config_binding *binding)
{
    if (binding == NULL || binding->product_config == NULL || binding->product_config_size != sizeof(fractal_atlas_config)) {
        return NULL;
    }

    return (const fractal_atlas_config *)binding->product_config;
}

static unsigned long fractal_atlas_step_interval(const screensave_saver_session *session)
{
    if (session == NULL) {
        return 66UL;
    }
...
static unsigned int fractal_atlas_hold_ticks(const screensave_saver_session *session)
{
    if (session == NULL) {
        return 80U;
    }
    ...
    switch (session->config.speed_mode) {
    case FRACTAL_ATLAS_SPEED_STILL:
        return session->config.mode == FRACTAL_ATLAS_MODE_ATLAS ? 170U : 120U;
    case FRACTAL_ATLAS_SPEED_BRISK:
        return 56U;
    case FRACTAL_ATLAS_SPEED_STANDARD:
    default:
        return session->config.mode == FRACTAL_ATLAS_MODE_ATLAS ? 110U : 78U;
    }
}

static unsigned int fractal_atlas_rows_per_step(const screensave_saver_session *session)
{
    unsigned int rows;
...
static int fractal_atlas_target_width(const screensave_saver_session *session)
{
    int width;
    ...
    if (session->config.refinement_mode == FRACTAL_ATLAS_REFINEMENT_DRAFT) {
        width -= 24;
    } else if (session->config.refinement_mode == FRACTAL_ATLAS_REFINEMENT_FINE) {
        width += 24;
    }
...
static void fractal_atlas_pick_field_size(screensave_saver_session *session)
{
    int width;
    int height;
    ...
    width = fractal_atlas_target_width(session);
...
static int fractal_atlas_iteration_limit(const screensave_saver_session *session)
{
    int limit;
    ...
    if (session->config.mode == FRACTAL_ATLAS_MODE_JULIA) {
        limit += 8;
    }
...
static void fractal_atlas_zero_escape_values(screensave_saver_session *session)
{
    size_t count;
...
static int fractal_atlas_allocate_escape_values(screensave_saver_session *session)
{
...
static const fractal_atlas_waypoint *fractal_atlas_active_route(const screensave_saver_session *session)
{
    if (session == NULL) {
        return g_fractal_atlas_atlas_points;
    }

    switch (session->config.mode) {
    case FRACTAL_ATLAS_MODE_VOYAGE:
        return g_fractal_atlas_voyage_points;
    case FRACTAL_ATLAS_MODE_JULIA:
        return g_fractal_atlas_julia_points;
...
static void fractal_atlas_refresh_view(screensave_saver_session *session)
{
    const fractal_atlas_waypoint *route;
    unsigned int index;
    ...
    route = fractal_atlas_active_route(session);
    index = session->route_index % FRACTAL_ATLAS_ROUTE_POINT_COUNT;
    session->current_view = route[index];
}

static void fractal_atlas_restart_refinement(screensave_saver_session *session)
{
    ...
    fractal_atlas_zero_escape_values(session);
    clear_color.red = 0;
    clear_color.green = 0;
    clear_color.blue = 0;
...
static int fractal_atlas_prepare_buffers(screensave_saver_session *session)
{
    if (session == NULL) {
        return 0;
    }

    fractal_atlas_pick_field_size(session);
    session->max_iterations = fractal_atlas_iteration_limit(session);
    ...
    if (!fractal_atlas_allocate_escape_values(session)) {
        return 0;
    }
...
static unsigned short fractal_atlas_sample_escape(
...
static void fractal_atlas_refine_rows(screensave_saver_session *session, unsigned int row_count)
{
    ...
    for (row_index = 0U; row_index < row_count; ++row_index) {
        ...
        for (x = 0; x < session->field_size.width; ++x) {
            session->escape_values[(y * session->field_size.width) + x] =
                fractal_atlas_sample_escape(session, x, y);
        }
        session->refinement_row += 1;
    }
}

static void fractal_atlas_advance_route(screensave_saver_session *session)
{
    if (session == NULL) {
        return;
    }

    session->route_index = (session->route_index + 1U) % FRACTAL_ATLAS_ROUTE_POINT_COUNT;
    session->route_phase = 0U;
    fractal_atlas_refresh_view(session);
    fractal_atlas_restart_refinement(session);
}

static void fractal_atlas_run_tick(screensave_saver_session *session)
{
    if (session == NULL) {
        return;
    }
    ...
    if (session->refinement_row < session->field_size.height) {
        fractal_atlas_refine_rows(session, fractal_atlas_rows_per_step(session));
        return;
    }

    session->route_phase += 1U;
    if (session->route_phase >= fractal_atlas_hold_ticks(session)) {
        fractal_atlas_advance_route(session);
    }
}

static void fractal_atlas_initialize_session(
...
)
{
    ...
    session->theme = fractal_atlas_resolve_theme(environment->config_binding);
    ...
    fractal_atlas_rng_seed(&session->rng, seed);
    session->step_accumulator = 0UL;
    session->phase_counter = 0UL;
    session->route_index = (unsigned int)fractal_atlas_rng_range(&session->rng, FRACTAL_ATLAS_ROUTE_POINT_COUNT);
    session->route_phase = 0U;
    session->palette_phase = (unsigned int)fractal_atlas_rng_range(&session->rng, 256UL);
    session->escape_values = NULL;
    ZeroMemory(&session->visual_buffer, sizeof(session->visual_buffer));

    if (!fractal_atlas_prepare_buffers(session)) {
...
unsigned long fractal_atlas_rng_range(fractal_atlas_rng_state *state, unsigned long upper_bound)
{
    ...
    return fractal_atlas_rng_next(state) % upper_bound;
}
...
)
{
    ...
    const fractal_atlas_config *bound_config;
    screensave_common_config safe_common;
    fractal_atlas_config safe_config;
    ...
    fractal_atlas_config_set_defaults(&safe_common, &config, sizeof(config));
    bound_config = fractal_atlas_resolve_config(environment->config_binding);
    ...
    safe_config = config;
    fractal_atlas_config_clamp(&safe_common, &safe_config, sizeof(safe_config));
    ...
    fractal_atlas_initialize_session(session, &safe_config, safe_common.detail_level, environment);
...
)
{
    ...
    delta = environment->clock.delta_millis != 0UL ? environment->clock.delta_millis : fractal_atlas_step_interval(session);
    interval = fractal_atlas_step_interval(session);
    session->step_accumulator += delta;
    while (session->step_accumulator >= interval) {
        session->step_accumulator -= interval;
        fractal_atlas_run_tick(session);
    }
}
