...
const screensave_theme_descriptor g_city_nocturne_themes[] = {
    {
        "quiet_city_sleep",
        "Quiet City Sleep",
...
{
    if (count_out != NULL) {
        *count_out = (unsigned int)(sizeof(g_city_nocturne_themes) / sizeof(g_city_nocturne_themes[0]));
    }

    return g_city_nocturne_themes;
}
...
const screensave_theme_descriptor *city_nocturne_find_theme_descriptor(const char *theme_key)
{
    ...
    themes = city_nocturne_get_themes(&theme_count);
