...
static const screensave_theme_descriptor *vector_resolve_theme(const screensave_config_binding *binding)
{
    const screensave_theme_descriptor *theme;

    if (binding != NULL && binding->common_config != NULL) {
        theme = vector_find_theme_descriptor(binding->common_config->theme_key);
        if (theme != NULL) {
            return theme;
        }
    }

    return vector_find_theme_descriptor(VECTOR_DEFAULT_THEME_KEY);
}

static const vector_config *vector_resolve_config(const screensave_config_binding *binding)
{
    if (binding == NULL || binding->product_config == NULL || binding->product_config_size != sizeof(vector_config)) {
        return NULL;
    }

    return (const vector_config *)binding->product_config;
}

static unsigned long vector_step_interval(const screensave_saver_session *session)
{
    if (session == NULL) {
        return 64UL;
    }
...
static unsigned int vector_object_target(const screensave_saver_session *session)
{
    unsigned int count;
    ...
    if (session->config.scene_mode == VECTOR_SCENE_TUNNEL) {
        count += 2U;
    }
    if (session->preview_mode && count > 10U) {
        count = 10U;
    }
    if (count > VECTOR_MAX_OBJECTS) {
        count = VECTOR_MAX_OBJECTS;
    }
...
static void vector_seed_object(
...
)
{
    ...
    object->phase = (unsigned int)vector_hash(salt + 1UL);
    object->twist = vector_signed_range(vector_hash(salt + 2UL), 512);
    object->scale = 18 + vector_signed_range(vector_hash(salt + 3UL), 8);

    switch (session->config.scene_mode) {
    case VECTOR_SCENE_TUNNEL:
        object->kind = (int)(index % 3U);
        object->x = vector_signed_range(vector_hash(salt + 4UL), 48);
        object->y = vector_signed_range(vector_hash(salt + 5UL), 28);
        object->z = base_offset * 8 + vector_signed_range(vector_hash(salt + 6UL), 32);
        break;

    case VECTOR_SCENE_TERRAIN:
        object->kind = (int)((index + 1U) % 4U);
        object->x = base_offset * 16 - 140 + vector_signed_range(vector_hash(salt + 4UL), 24);
        object->y = vector_signed_range(vector_hash(salt + 5UL), 72);
        object->z = vector_signed_range(vector_hash(salt + 6UL), 240);
        break;

    case VECTOR_SCENE_FIELD:
    default:
        object->kind = (int)(index % 3U);
        object->x = vector_signed_range(vector_hash(salt + 4UL), 160);
        object->y = vector_signed_range(vector_hash(salt + 5UL), 96);
        object->z = base_offset * 12 + vector_signed_range(vector_hash(salt + 6UL), 90);
        break;
    }
}

static void vector_initialize_scene(screensave_saver_session *session)
{
    unsigned int index;
    ...
    session->object_count = vector_object_target(session);
    for (index = 0U; index < session->object_count; ++index) {
        vector_seed_object(session, &session->objects[index], index);
    }
}

static void vector_initialize_session(
    screensave_saver_session *session,
...
)
{
    ...
    vector_rng_seed(&session->rng, session->scene_seed);
...
static void vector_mutate_scene(screensave_saver_session *session)
{
    vector_object *object;
    unsigned int index;
    int drift;
    ...
    index = (unsigned int)vector_rng_range(&session->rng, (unsigned long)session->object_count);
    object = &session->objects[index];
    drift = vector_signed_range(vector_rng_next(&session->rng), 14);
    object->phase += (unsigned int)vector_rng_range(&session->rng, 32UL);
    object->twist += drift;
    object->scale += vector_signed_range(vector_rng_next(&session->rng), 2);
    if (object->scale < 10) {
        object->scale = 10;
    }
    if (object->scale > 36) {
    ...
    if (session->config.scene_mode == VECTOR_SCENE_TUNNEL) {
        object->x += vector_signed_range(vector_rng_next(&session->rng), 6);
        object->y += vector_signed_range(vector_rng_next(&session->rng), 4);
    } else if (session->config.scene_mode == VECTOR_SCENE_TERRAIN) {
        object->y += vector_signed_range(vector_rng_next(&session->rng), 8);
    } else {
        object->x += vector_signed_range(vector_rng_next(&session->rng), 10);
        object->z += vector_signed_range(vector_rng_next(&session->rng), 12);
    }

    session->scene_variant = (session->scene_variant + 1UL + (unsigned long)vector_rng_range(&session->rng, 3UL)) & 255UL;
}
...
unsigned long vector_rng_range(vector_rng_state *state, unsigned long upper_bound)
{
    ...
    return vector_rng_next(state) % upper_bound;
}
...
)
{
    ...
    const vector_config *bound_config;
    ...
    vector_config_set_defaults(&safe_common, &config, sizeof(config));
    bound_config = vector_resolve_config(environment->config_binding);
    ...
    safe_config = config;
    vector_config_clamp(&safe_common, &safe_config, sizeof(safe_config));
    ...
    vector_initialize_session(session, &safe_config, environment);
    *session_out = session;
    return 1;
}
...
void vector_resize_session(screensave_saver_session *session, const screensave_saver_environment *environment)
{
