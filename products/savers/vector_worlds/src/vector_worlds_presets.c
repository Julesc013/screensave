...
const screensave_preset_descriptor g_vector_worlds_presets[] = {
    {
        "quiet_vector_hall",
        "Quiet Vector Hall",
...
static const vector_worlds_preset_values g_vector_worlds_preset_values[] = {
    { VECTOR_WORLDS_SCENE_FIELD, VECTOR_WORLDS_SPEED_CALM, VECTOR_WORLDS_DETAIL_STANDARD },
    { VECTOR_WORLDS_SCENE_FIELD, VECTOR_WORLDS_SPEED_CALM, VECTOR_WORLDS_DETAIL_LOW },
    { VECTOR_WORLDS_SCENE_TUNNEL, VECTOR_WORLDS_SPEED_STANDARD, VECTOR_WORLDS_DETAIL_STANDARD },
    { VECTOR_WORLDS_SCENE_TERRAIN, VECTOR_WORLDS_SPEED_STANDARD, VECTOR_WORLDS_DETAIL_HIGH },
    { VECTOR_WORLDS_SCENE_FIELD, VECTOR_WORLDS_SPEED_CALM, VECTOR_WORLDS_DETAIL_LOW }
};
...
{
    if (count_out != NULL) {
        *count_out = (unsigned int)(sizeof(g_vector_worlds_presets) / sizeof(g_vector_worlds_presets[0]));
    }

    return g_vector_worlds_presets;
}
...
const vector_worlds_preset_values *vector_worlds_find_preset_values(const char *preset_key)
{
    ...
    for (index = 0U; index < preset_count; ++index) {
        if (presets[index].preset_key != NULL && strcmp(presets[index].preset_key, preset_key) == 0) {
            return &g_vector_worlds_preset_values[index];
        }
    }
...
)
{
    ...
    const vector_worlds_preset_values *preset_values;
    ...
    preset_values = vector_worlds_find_preset_values(preset_key);
