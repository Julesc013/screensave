...
const screensave_theme_descriptor g_vector_worlds_themes[] = {
    {
        "quiet_midnight_grid",
        "Quiet Midnight Grid",
...
{
    if (count_out != NULL) {
        *count_out = (unsigned int)(sizeof(g_vector_worlds_themes) / sizeof(g_vector_worlds_themes[0]));
    }

    return g_vector_worlds_themes;
}
...
const screensave_theme_descriptor *vector_worlds_find_theme_descriptor(const char *theme_key)
{
    ...
    themes = vector_worlds_get_themes(&theme_count);
