#include "../src/vector_worlds_internal.h"
...
int main(void)
{
    ...
    module = vector_worlds_get_module();
    ...
    vector_worlds_config_clamp(&common_config, &product_config, sizeof(product_config));
    if (!screensave_common_config_validate(&common_config, &issue_flags)) {
        return 2;
    }

    if (vector_worlds_find_preset_values("sgi_neon_tunnel") == NULL) {
        return 3;
    }

    if (vector_worlds_find_theme_descriptor("quiet_midnight_grid") == NULL) {
        return 4;
    }
    ...
    session = NULL;
    if (!vector_worlds_create_session(module, &session, &environment) || session == NULL) {
        return 5;
    }

    environment.clock.delta_millis = 96UL;
    vector_worlds_step_session(session, &environment);
    if (session->theme == NULL) {
        vector_worlds_destroy_session(session);
        return 6;
    }
