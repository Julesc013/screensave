#include "../src/gl_gallery_internal.h"
...
int main(void)
{
    ...
    module = gl_gallery_get_module();
    if (!screensave_saver_module_is_valid(module)) {
        return 1;
    }

    gl_gallery_config_set_defaults(&common_config, &product_config, sizeof(product_config));
    gl_gallery_config_clamp(&common_config, &product_config, sizeof(product_config));
    ...
    if ((module->capability_flags & SCREENSAVE_SAVER_CAP_OPTIONAL_GL11) == 0UL) {
        return 4;
    }
