...
const screensave_theme_descriptor g_gl_gallery_themes[] = {
    {
...
{
    if (count_out != NULL) {
        *count_out = (unsigned int)(sizeof(g_gl_gallery_themes) / sizeof(g_gl_gallery_themes[0]));
    }

    return g_gl_gallery_themes;
}
...
const screensave_theme_descriptor *gl_gallery_find_theme_descriptor(const char *theme_key)
{
    ...
    themes = gl_gallery_get_themes(&theme_count);
