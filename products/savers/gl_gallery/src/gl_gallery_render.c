...
static gl_gallery_renderer_tier gl_gallery_tier_from_info(const screensave_renderer_info *info)
{
    if (info == NULL) {
...
static screensave_color gl_gallery_mix(
...
static screensave_color gl_gallery_background_color(const screensave_saver_session *session, gl_gallery_renderer_tier tier)
{
    ...
    if (session->config.scene_mode == GL_GALLERY_SCENE_NEON) {
    ...
    } else if (session->config.scene_mode == GL_GALLERY_SCENE_SHOWCASE) {
    ...
    } else if (tier == GL_GALLERY_TIER_ADVANCED) {
        color = session->theme->primary_color;
        color.red = (unsigned char)(color.red / 4U);
        color.green = (unsigned char)(color.green / 4U);
        color.blue = (unsigned char)(color.blue / 4U);
...
static void gl_gallery_draw_fill(
...
static void gl_gallery_draw_frame(
    screensave_renderer *renderer,
    int left,
    int top,
    int width,
...
static void gl_gallery_point_on_circle(
    int center_x,
    int center_y,
    int radius_x,
    int radius_y,
...
static void gl_gallery_draw_ring(
    screensave_renderer *renderer,
    int center_x,
    int center_y,
    int radius_x,
...
static int gl_gallery_marker_visual_count(const screensave_saver_session *session, gl_gallery_renderer_tier tier)
{
    int count;
...
static void gl_gallery_draw_marker_field(
...
)
{
    ...
    count = gl_gallery_marker_visual_count(session, tier);
    for (index = 0; index < count; ++index) {
        const gl_gallery_marker *marker;
        int offset_x;
        int offset_y;
        int radius;
        ...
        gl_gallery_draw_ring(
            renderer,
            marker->x + offset_x,
            marker->y + offset_y,
            radius + 4,
            radius + 2,
            tier == GL_GALLERY_TIER_ADVANCED ? 12U : 8U,
...
static void gl_gallery_draw_compatibility_scene(
    screensave_saver_session *session,
    screensave_renderer *renderer,
    gl_gallery_renderer_tier tier
)
{
    ...
    back = gl_gallery_background_color(session, tier);
    frame = gl_gallery_mix(back, session->theme->primary_color, 88U);
    accent = gl_gallery_mix(back, session->theme->accent_color, 174U);
    glow = gl_gallery_mix(frame, session->theme->accent_color, tier == GL_GALLERY_TIER_ADVANCED ? 184U : 128U);
    ...
    for (index = 0; index < panel_count; ++index) {
        ...
        gl_gallery_draw_frame(renderer, left, panel_top, panel_width, panel_height, frame);
        gl_gallery_draw_fill(renderer, left + 4, panel_top + 4, panel_width - 8, panel_height - 8, glow);
        gl_gallery_draw_ring(
            renderer,
            left + panel_width / 2,
    ...
    gl_gallery_draw_marker_field(session, renderer, tier, frame, glow);
}

static void gl_gallery_draw_neon_scene(
    screensave_saver_session *session,
    screensave_renderer *renderer,
    gl_gallery_renderer_tier tier
)
{
    ...
    back = gl_gallery_background_color(session, tier);
    neon_a = gl_gallery_mix(back, session->theme->primary_color, 144U);
    neon_b = gl_gallery_mix(back, session->theme->accent_color, 192U);
    neon_c = gl_gallery_mix(neon_a, session->theme->accent_color, tier == GL_GALLERY_TIER_ADVANCED ? 160U : 112U);
    ...
    for (index = 0; index < sweep_count; ++index) {
        ...
        gl_gallery_draw_frame(renderer, x_offset + 8, y - 8, 60 + (index * 8), 20, neon_b);
        if (tier != GL_GALLERY_TIER_GDI || index % 2 == 0) {
            gl_gallery_draw_ring(
                renderer,
    ...
    gl_gallery_draw_marker_field(session, renderer, tier, neon_a, neon_c);
}

static void gl_gallery_draw_showcase_scene(
    screensave_saver_session *session,
    screensave_renderer *renderer,
    gl_gallery_renderer_tier tier
)
{
    ...
    back = gl_gallery_background_color(session, tier);
    frame = gl_gallery_mix(back, session->theme->primary_color, 120U);
    highlight = gl_gallery_mix(back, session->theme->accent_color, 178U);
    beam = gl_gallery_mix(frame, session->theme->accent_color, tier == GL_GALLERY_TIER_ADVANCED ? 200U : 148U);
    ...
    for (index = 0; index < layer_count; ++index) {
        ...
        gl_gallery_draw_frame(renderer, inset, top, width, height, frame);
        gl_gallery_draw_fill(renderer, inset + 3, top + 3, width - 6, height - 6, highlight);
        gl_gallery_draw_ring(
            renderer,
    ...
    gl_gallery_draw_marker_field(session, renderer, tier, frame, beam);
}
...
)
{
    ...
    tier = gl_gallery_tier_from_info(&info);

    switch (session->config.scene_mode) {
    case GL_GALLERY_SCENE_NEON:
        gl_gallery_draw_neon_scene(session, renderer, tier);
        break;
    case GL_GALLERY_SCENE_SHOWCASE:
        gl_gallery_draw_showcase_scene(session, renderer, tier);
        break;
    case GL_GALLERY_SCENE_COMPATIBILITY:
    default:
        gl_gallery_draw_compatibility_scene(session, renderer, tier);
        break;
    }
}
