...
const screensave_theme_descriptor g_signal_lab_themes[] = {
    {
        "green_terminal",
        "Green Terminal",
...
{
    if (count_out != NULL) {
        *count_out = (unsigned int)(sizeof(g_signal_lab_themes) / sizeof(g_signal_lab_themes[0]));
    }

    return g_signal_lab_themes;
}
...
const screensave_theme_descriptor *signal_lab_find_theme_descriptor(const char *theme_key)
{
    ...
    themes = signal_lab_get_themes(&theme_count);
