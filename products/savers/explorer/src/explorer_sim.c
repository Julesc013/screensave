...
static const screensave_theme_descriptor *explorer_resolve_theme(
    const screensave_config_binding *binding
)
{
    if (binding != NULL && binding->common_config != NULL) {
        return explorer_find_theme_descriptor(binding->common_config->theme_key);
    }

    return explorer_find_theme_descriptor(EXPLORER_DEFAULT_THEME_KEY);
}

static const explorer_config *explorer_resolve_config(const screensave_config_binding *binding)
{
    if (binding == NULL || binding->product_config == NULL || binding->product_config_size != sizeof(explorer_config)) {
        return NULL;
    }

    return (const explorer_config *)binding->product_config;
}

static unsigned int explorer_target_segment_count(const screensave_saver_session *session)
{
    unsigned int count;
    ...
    if (session->config.scene_mode == EXPLORER_SCENE_CANYON && count < 11U) {
        count += 1U;
    }
    if (session->preview_mode && count > 8U) {
        count = 8U;
    }
    if (count > EXPLORER_MAX_SEGMENTS) {
        count = EXPLORER_MAX_SEGMENTS;
    }

    return count;
}

static unsigned long explorer_step_interval(const screensave_saver_session *session)
{
    ...
    if (session->config.scene_mode == EXPLORER_SCENE_CANYON) {
        interval += 8UL;
    } else if (session->config.scene_mode == EXPLORER_SCENE_INDUSTRIAL && interval > 8UL) {
        interval -= 4UL;
    }
...
)
{
    ...
    segment->variant = (int)explorer_rng_range(&session->rng, 3UL);
    ...
    case EXPLORER_SCENE_INDUSTRIAL:
        ...
        curve_limit = 4;
        length_base = 10UL;
        segment->light_interval = 2 + (int)explorer_rng_range(&session->rng, 2UL);
        segment->sign_interval = 2 + (int)explorer_rng_range(&session->rng, 3UL);
        break;
    case EXPLORER_SCENE_CANYON:
        ...
        curve_limit = 5;
        length_base = 11UL;
        segment->light_interval = 4 + (int)explorer_rng_range(&session->rng, 3UL);
    ...
    default:
        ...
        curve_limit = 3;
        length_base = 12UL;
        segment->light_interval = 2 + (int)explorer_rng_range(&session->rng, 3UL);
        segment->sign_interval = 3 + (int)explorer_rng_range(&session->rng, 3UL);
        break;
    }

    segment->length_steps = (unsigned int)(length_base + explorer_rng_range(&session->rng, 8UL));
    if (session->config.speed_mode == EXPLORER_SPEED_CALM) {
        segment->length_steps += 4U;
    } else if (session->config.speed_mode == EXPLORER_SPEED_BRISK && segment->length_steps > 6U) {
        segment->length_steps -= 2U;
    }

    segment->width = width_base + (int)explorer_rng_range(&session->rng, 12UL);
    segment->height = height_base + (int)explorer_rng_range(&session->rng, 12UL);
    segment->curve = (int)explorer_rng_range(&session->rng, (unsigned long)(curve_limit * 2 + 1)) - curve_limit;
    segment->openness = openness_base + (int)explorer_rng_range(&session->rng, 8UL);
...
static void explorer_build_route(screensave_saver_session *session, unsigned long route_seed)
{
    unsigned int index;
    unsigned int count;
    ...
    explorer_rng_seed(&session->rng, route_seed);
    count = explorer_target_segment_count(session);
    session->segment_count = count;
    session->current_segment_index = 0U;
    session->current_segment_step = 0U;
    session->camera_offset = (int)explorer_rng_range(&session->rng, 12UL) - 6;
    session->portal_pulse = 8UL;

    for (index = 0U; index < count; ++index) {
        explorer_seed_segment(session, &session->segments[index], index);
    }
}

static void explorer_advance_route(screensave_saver_session *session)
{
...
unsigned long explorer_rng_range(explorer_rng_state *state, unsigned long upper_bound)
{
    ...
    return explorer_rng_next(state) % upper_bound;
}
...
)
{
    ...
    const explorer_config *bound_config;
    ...
    explorer_config_set_defaults(&safe_common, &config, sizeof(config));
    bound_config = explorer_resolve_config(environment->config_binding);
    ...
    explorer_config_clamp(&safe_common, &config, sizeof(config));
...
)
{
    ...
    interval = explorer_step_interval(session);
    ...
    while (session->travel_millis >= interval) {
        ...
        explorer_advance_route(session);
    }
}
