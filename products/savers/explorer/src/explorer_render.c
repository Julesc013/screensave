...
#define EXPLORER_RENDER_BANDS 12U

static int explorer_clamp_int(int value, int min_value, int max_value)
{
    if (value < min_value) {
        return min_value;
    }
...
static screensave_color explorer_make_color(unsigned char red, unsigned char green, unsigned char blue)
{
...
static screensave_color explorer_mix_color(
...
static screensave_color explorer_scale_color(screensave_color color, unsigned int scale)
{
    if (scale > 255U) {
        scale = 255U;
    }
...
static screensave_color explorer_scene_background_color(const screensave_saver_session *session)
{
    screensave_color color;

    color = explorer_make_color(12, 14, 18);
    if (session == NULL || session->theme == NULL) {
        return color;
    }

    if (lstrcmpiA(session->theme->theme_key, "cold_lab") == 0) {
        color = explorer_scale_color(session->theme->primary_color, 36U);
    } else if (lstrcmpiA(session->theme->theme_key, "industrial_tunnel") == 0) {
        color = explorer_scale_color(session->theme->primary_color, 30U);
    } else if (lstrcmpiA(session->theme->theme_key, "neon_maze") == 0) {
        color = explorer_scale_color(session->theme->primary_color, 24U);
    } else if (lstrcmpiA(session->theme->theme_key, "dusty_ruin") == 0) {
        color = explorer_scale_color(session->theme->primary_color, 28U);
    } else if (lstrcmpiA(session->theme->theme_key, "quiet_night_run") == 0) {
        color = explorer_scale_color(session->theme->primary_color, 40U);
    }

    return color;
}

static int explorer_scene_horizon(const screensave_saver_session *session, const explorer_segment *segment)
{
    int horizon;
    ...
    horizon = explorer_clamp_int(horizon, session->drawable_size.height / 5, session->drawable_size.height / 2);
    return horizon;
}

static int explorer_slice_count(const screensave_saver_session *session)
{
    int slices;
    ...
    if (session->config.scene_mode == EXPLORER_SCENE_CANYON) {
        slices += 1;
    }
    if (session->preview_mode && slices > 8) {
        slices = 8;
    }
    if (slices > (int)EXPLORER_RENDER_BANDS) {
        slices = (int)EXPLORER_RENDER_BANDS;
    }

    return slices;
}

static screensave_color explorer_band_color(
    const screensave_saver_session *session,
    const explorer_segment *segment,
    int band_index,
    int band_count,
    int is_path_band
)
{
    ...
    if (is_path_band) {
        color = explorer_mix_color(base_color, path_color, 80U + amount / 2U + pulse / 2U);
    } else {
        color = explorer_mix_color(base_color, wall_color, 44U + amount / 2U);
    }

    if (session != NULL && session->config.scene_mode == EXPLORER_SCENE_CANYON && !is_path_band) {
        color = explorer_mix_color(color, explorer_make_color(128, 84, 48), 48U + amount / 4U);
    } else if (session != NULL && session->config.scene_mode == EXPLORER_SCENE_INDUSTRIAL && !is_path_band) {
        color = explorer_mix_color(color, explorer_make_color(32, 40, 44), 48U + amount / 4U);
    }

    if (session != NULL && session->config.scene_mode == EXPLORER_SCENE_CORRIDOR && is_path_band) {
        color = explorer_mix_color(color, explorer_make_color(84, 100, 118), 32U + pulse / 3U);
    }

    return color;
}

static void explorer_fill_clamped_rect(
...
static void explorer_draw_feature_light(
    screensave_renderer *renderer,
    int x,
    int y,
    int size,
...
static void explorer_draw_route_surface(
...
{
    screensave_renderer *renderer;
    screensave_pointi left_points[EXPLORER_RENDER_BANDS + 1U];
    screensave_pointi right_points[EXPLORER_RENDER_BANDS + 1U];
    int slice_count;
    int horizon;
    int floor_y;
    int band_index;
    ...
    screensave_pointi route_points[EXPLORER_RENDER_BANDS + 1U];
    ...
    slice_count = explorer_slice_count(session);
    horizon = explorer_scene_horizon(session, segment);
    floor_y = session->drawable_size.height - 4;
    center_x = (session->drawable_size.width / 2) + session->camera_offset;
    outline_color = explorer_mix_color(session->theme->primary_color, explorer_scene_background_color(session), 96U);
    highlight_color = explorer_mix_color(session->theme->accent_color, explorer_make_color(255, 255, 255), 48U);
    path_light = explorer_mix_color(session->theme->accent_color, explorer_make_color(255, 255, 255), 112U);
    sign_color = explorer_mix_color(session->theme->accent_color, explorer_make_color(255, 255, 255), 160U);
    ...
    for (band_index = 0; band_index <= slice_count; ++band_index) {
        ...
        } else if (session->config.scene_mode == EXPLORER_SCENE_INDUSTRIAL) {
        ...
        band_color = explorer_band_color(session, segment, band_index, slice_count, 1);
        explorer_fill_clamped_rect(
            renderer,
        ...
        explorer_fill_clamped_rect(
            ...
            left_points[band_index].x,
            panel_bottom,
            explorer_band_color(session, segment, band_index, slice_count, 0),
            session
        );
        explorer_fill_clamped_rect(
            renderer,
        ...
        if (segment->light_interval > 0 && ((unsigned int)band_index + session->current_segment_step) % (unsigned int)segment->light_interval == 0U) {
            ...
            light_size = session->config.scene_mode == EXPLORER_SCENE_CORRIDOR ? 2 : 3;
            light_x = side < 0 ? left_points[band_index].x + 2 : right_points[band_index].x - 4;
            explorer_draw_feature_light(
                renderer,
                light_x,
                light_y,
                light_size,
                explorer_mix_color(highlight_color, path_light, session->portal_pulse > 0UL ? 96U : 48U)
            );
        }

        if (
            segment->sign_interval > 0 &&
            session->config.scene_mode != EXPLORER_SCENE_CANYON &&
        ...
        if (session->config.scene_mode == EXPLORER_SCENE_CANYON && (band_index & 1) == 0) {
            int ridge_width;
            int ridge_height;
            int ridge_x;
            ...
            explorer_fill_clamped_rect(
                ...
                explorer_mix_color(band_color, explorer_make_color(224, 164, 92), 72U),
                session
            );
        }
    }
    ...
    if (session->portal_pulse > 0UL) {
        ...
        screensave_renderer_fill_rect(
            renderer,
            &portal_rect,
            explorer_mix_color(highlight_color, session->theme->accent_color, 160U)
        );
    }
}
...
)
{
    ...
    background = explorer_scene_background_color(session);
    screensave_renderer_clear(environment->renderer, background);

    if (session->config.scene_mode == EXPLORER_SCENE_CANYON) {
        sky_tint = explorer_mix_color(background, session->theme->accent_color, 64U);
    } else if (session->config.scene_mode == EXPLORER_SCENE_INDUSTRIAL) {
        sky_tint = explorer_mix_color(background, session->theme->primary_color, 48U);
    } else {
        sky_tint = explorer_mix_color(background, session->theme->accent_color, 40U);
    }

    sky_rect.x = 0;
    sky_rect.y = 0;
    sky_rect.width = session->drawable_size.width;
    sky_rect.height = explorer_scene_horizon(session, segment);
    screensave_renderer_fill_rect(environment->renderer, &sky_rect, sky_tint);

    explorer_draw_route_surface(session, environment, segment);
}
