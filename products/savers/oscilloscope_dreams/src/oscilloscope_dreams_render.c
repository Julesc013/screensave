...
static screensave_color oscilloscope_dreams_scale_color(
    screensave_color color,
    unsigned int scale
)
{
...
)
{
    ...
    screensave_renderer_clear(environment->renderer, oscilloscope_dreams_background_color(session));
    ...
    accent_color = oscilloscope_dreams_scale_color(session->theme->accent_color, 72U);
    frame_rect.x = 10;
    frame_rect.y = 10;
