...
const screensave_theme_descriptor g_oscilloscope_dreams_themes[] = {
    {
        "amber_scope",
        "Amber Scope",
...
{
    if (count_out != NULL) {
        *count_out = (unsigned int)(sizeof(g_oscilloscope_dreams_themes) / sizeof(g_oscilloscope_dreams_themes[0]));
    }

    return g_oscilloscope_dreams_themes;
}
...
const screensave_theme_descriptor *oscilloscope_dreams_find_theme_descriptor(const char *theme_key)
{
    ...
    themes = oscilloscope_dreams_get_themes(&theme_count);
