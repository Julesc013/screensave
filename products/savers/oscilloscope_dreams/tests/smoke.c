#include "../src/oscilloscope_dreams_internal.h"
...
int main(void)
{
    ...
    module = oscilloscope_dreams_get_module();
    ...
    oscilloscope_dreams_config_clamp(&common_config, &product_config, sizeof(product_config));
    if (!screensave_common_config_validate(&common_config, &issue_flags)) {
        return 2;
    }

    if (oscilloscope_dreams_find_preset_values("green_vector") == NULL) {
        return 3;
    }

    if (oscilloscope_dreams_find_theme_descriptor("blue_lab") == NULL) {
        return 4;
    }
    ...
    session = NULL;
    if (!oscilloscope_dreams_create_session(module, &session, &environment) || session == NULL) {
        return 5;
    }

    environment.clock.delta_millis = 33UL;
    oscilloscope_dreams_step_session(session, &environment);
