...
| S08 | Optional GL11 backend and runtime selection | Conservative GL11 backend, capability capture, renderer selection, explicit fallback, BenchLab validation | S03-S07 | `platform/`, `products/apps/benchlab/`, `build/`, targeted docs, tests | GDI remains mandatory, no GL-plus, no product-level GL dependency |
