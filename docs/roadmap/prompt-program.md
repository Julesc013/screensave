...
| S04 | Public platform surface, shared config resolution, and initial GDI floor | Initial reusable headers, config loading path, preset/theme resolution scaffolding, lifecycle contracts tightened, and first universal renderer-floor scaffolding | S02-S03 | `platform/`, `specs/`, `tests/`, docs | Product-local ownership, deterministic seed rules |
