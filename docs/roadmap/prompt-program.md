...
| S01 | Public platform surface and header planning | Initial public headers, platform skeleton notes, host-facing boundary cleanup | S00 | `platform/`, `specs/`, `docs/`, `tests/` | C89 baseline, ANSI host path, product isolation |
| S02 | Win32 host skeleton and classic mode routing | Host scaffolding for `/s`, `/c`, `/p`, shared mode dispatch, config-entry stubs | S01 | `platform/`, `tests/`, `validation/`, relevant docs | No Unicode-only baseline, no renderer assumptions beyond GDI |
| S03 | GDI universal renderer floor | GDI backend scaffolding, frame loop basics, first baseline rendering tests | S02 | `platform/`, `tests/`, `validation/`, docs | GDI first, no GL requirement |
