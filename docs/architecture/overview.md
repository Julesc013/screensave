...
Post-`S15` continuation planning is tracked in `docs/roadmap/post-s15-plan.md`, and the near-term continuation path is now the completed rename, checkpoint, productization, shared-settings, and Windows-integration sequence followed by the portable distribution bundle rather than more naming cleanup or host-lifecycle renegotiation.
