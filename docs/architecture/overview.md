...
Post-`S15` continuation planning is tracked in `docs/roadmap/post-s15-plan.md`, and the near-term continuation path is now the completed rename, checkpoint, productization, shared-settings, Windows-integration, portable-bundle, installer, suite-meta-saver, suite-app, and SDK sequence before backlog ingestion rather than more naming cleanup or host-lifecycle renegotiation.
