...
Series 15 proves the same contracts can also host a three-tier renderer stack by adding the optional capability-gated GL-plus backend plus GL Gallery without weakening the GDI baseline, the conservative GL11 tier, or the product/local ownership boundaries.
