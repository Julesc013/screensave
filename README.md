...
- The current `C14` portable bundle is the pre-freeze Core candidate surface, not the frozen Core baseline.
...
Portable bundle sources now live under `packaging/portable/`, and the current refreshed portable output lives under `out/portable/screensave-portable-c14-rc/` with a matching zip beside it. Under the `C15` doctrine that staged bundle is the current Core candidate surface. Installed-distribution sources now also live under `packaging/installer/`, and the current refreshed installer output lives under `out/installer/screensave-installer-c14-rc/` with a matching zip beside it. Under the same doctrine that staged installer is an optional companion channel built on top of Core. Both delivery paths remain intentionally truthful and partial in this environment: the configured local output roots currently expose only `nocturne.scr`, `ricochet.scr`, `deepfield.scr`, `pipeworks.scr`, `lifeforms.scr`, and `benchlab.exe`, so `anthology.scr`, `suite.exe`, and the remaining saver binaries are represented in release-readiness notes but not in the staged payloads.
