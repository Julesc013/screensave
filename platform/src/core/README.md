...
- `substrate/` for private renderer-substrate routing, backend descriptors, capability capture, present-path policy, and future optional service seams introduced in `SX01`
...
Series 15 adds only narrow tiered renderer-selection and fallback-reporting glue here so GDI remains the guaranteed floor, GL11 remains the conservative accelerated middle tier, GL21 stays strictly optional and capability-gated, later GL33 and GL46 tiers can stay explicit without pretending to exist, and the null backend remains an internal emergency fallback only.
