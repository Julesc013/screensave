...
static int screensave_backend_registry_capture_caps_gl46(
    const screensave_renderer *renderer,
    screensave_backend_caps *caps_out
)
{
    ...
    if (!screensave_gl46_state_from_renderer((screensave_renderer *)renderer, &state)) {
        return 0;
    }
    ...
    if (state->caps.premium_context) {
        caps_out->state_flags |= SCREENSAVE_BACKEND_CAPS_STATE_PREMIUM_CONTEXT;
    }
    if (state->caps.compatibility_profile) {
        caps_out->state_flags |= SCREENSAVE_BACKEND_CAPS_STATE_COMPAT_PROFILE;
    }
...
static int screensave_backend_registry_resize_gl46(
    screensave_renderer *renderer,
    const screensave_sizei *drawable_size
)
{
    return screensave_gl46_renderer_resize(renderer, drawable_size);
}

static void screensave_backend_registry_set_present_dc_gl46(
    screensave_renderer *renderer,
    HDC present_dc
)
{
    screensave_gl46_renderer_set_present_dc(renderer, present_dc);
}

static void screensave_backend_registry_clear_present_dc_gl46(screensave_renderer *renderer)
{
    screensave_gl46_renderer_clear_present_dc(renderer);
}
...
static const screensave_backend_descriptor g_screensave_backend_descriptors[] = {
    ...
    },
    {
        ...
        screensave_backend_registry_resize_gl46,
        screensave_backend_registry_set_present_dc_gl46,
        screensave_backend_registry_clear_present_dc_gl46,
        screensave_backend_registry_capture_caps_gl46
    },
    {
        SCREENSAVE_BACKEND_KIND_NULL,
