...
)
{
    ...
    if (pixels == NULL) {
        screensave_gl33_emit_diag(
            ...
            6987UL,
            "gl33_primitives",
            "GL33 bitmap upload memory allocation failed."
        );
        return 0;
    }

    screensave_gl33_convert_bitmap_to_rgba(bitmap, pixels);
    ...
    if (texture_id == 0U) {
        ...
        screensave_gl33_emit_diag(
            ...
            6988UL,
            "gl33_primitives",
            "glGenTextures failed for a GL33 bitmap blit."
        );
        return 0;
    }
    ...
    white.blue = 255U;
    white.alpha = 255U;
    screensave_gl33_init_vertex(&vertices[0], left, top, white, 0.0f, 1.0f);
    screensave_gl33_init_vertex(&vertices[1], right, top, white, 1.0f, 1.0f);
    screensave_gl33_init_vertex(&vertices[2], right, bottom, white, 1.0f, 0.0f);
    screensave_gl33_init_vertex(&vertices[3], left, top, white, 0.0f, 1.0f);
    screensave_gl33_init_vertex(&vertices[4], right, bottom, white, 1.0f, 0.0f);
    screensave_gl33_init_vertex(&vertices[5], left, bottom, white, 0.0f, 0.0f);
    (void)screensave_gl33_draw_vertices(
        state,
        state->texture_program,
        state->texture_viewport_location,
