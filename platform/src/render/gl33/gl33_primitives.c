#include "gl33_internal.h"

static const char *g_screensave_gl33_vertex_shader =
    "#version 330 compatibility\n"
    "layout(location=0) in vec2 a_pos;\n"
    "layout(location=1) in vec4 a_color;\n"
    "layout(location=2) in vec2 a_uv;\n"
    "out vec4 v_color;\n"
    "out vec2 v_uv;\n"
    "uniform vec2 u_viewport;\n"
    "void main(void)\n"
    "{\n"
    "    vec2 ndc;\n"
    "    ndc.x = (a_pos.x / u_viewport.x) * 2.0 - 1.0;\n"
    "    ndc.y = 1.0 - (a_pos.y / u_viewport.y) * 2.0;\n"
    "    gl_Position = vec4(ndc, 0.0, 1.0);\n"
    "    v_color = a_color;\n"
    "    v_uv = a_uv;\n"
    "}\n";

static const char *g_screensave_gl33_color_fragment_shader =
    "#version 330 compatibility\n"
    "in vec4 v_color;\n"
    "out vec4 f_color;\n"
    "void main(void)\n"
    "{\n"
    "    f_color = v_color;\n"
    "}\n";

static const char *g_screensave_gl33_texture_fragment_shader =
    "#version 330 compatibility\n"
    "in vec4 v_color;\n"
    "in vec2 v_uv;\n"
    "uniform sampler2D u_texture;\n"
    "out vec4 f_color;\n"
    "void main(void)\n"
    "{\n"
    "    f_color = texture(u_texture, v_uv) * v_color;\n"
    "}\n";

static int screensave_gl33_can_draw(
    screensave_renderer *renderer,
    screensave_gl33_state **state_out
)
{
    screensave_gl33_state *state;

    if (!screensave_gl33_state_from_renderer(renderer, &state)) {
        return 0;
    }

    if (!state->frame_open) {
        return 0;
    }

    if (state_out != NULL) {
        *state_out = state;
    }
    return 1;
}

static void screensave_gl33_init_vertex(
    screensave_gl33_vertex *vertex,
    GLfloat x,
    GLfloat y,
    screensave_color color,
    GLfloat u,
    GLfloat v
)
{
    if (vertex == NULL) {
        return;
    }

    vertex->x = x;
    vertex->y = y;
    vertex->red = color.red;
    vertex->green = color.green;
    vertex->blue = color.blue;
    vertex->alpha = color.alpha;
    vertex->u = u;
    vertex->v = v;
}

static int screensave_gl33_compile_shader(
    screensave_gl33_state *state,
    GLenum shader_type,
    const char *source_text,
    GLuint *shader_out
)
{
    GLuint shader;
    GLint compile_ok;
    char info_log[256];
    const char *text_array[1];

    if (shader_out != NULL) {
        *shader_out = 0U;
    }

    if (state == NULL || source_text == NULL) {
        return 0;
    }

    shader = state->create_shader_fn(shader_type);
    if (shader == 0U) {
        screensave_gl33_emit_diag(
            state,
            SCREENSAVE_DIAG_LEVEL_ERROR,
            6981UL,
            "gl33_primitives",
            "glCreateShader failed while preparing the GL33 shader pipeline."
        );
        return 0;
    }

    text_array[0] = source_text;
    state->shader_source_fn(shader, 1, text_array, NULL);
    state->compile_shader_fn(shader);
    compile_ok = 0;
    state->get_shader_iv_fn(shader, GL_COMPILE_STATUS, &compile_ok);
    if (compile_ok == 0) {
        info_log[0] = '\0';
        state->get_shader_info_log_fn(
            shader,
            (GLsizei)sizeof(info_log),
            NULL,
            info_log
        );
        screensave_gl33_emit_diag(
            state,
            SCREENSAVE_DIAG_LEVEL_ERROR,
            6982UL,
            "gl33_primitives",
            info_log[0] != '\0'
                ? info_log
                : "The GL33 shader pipeline could not compile a required shader."
        );
        state->delete_shader_fn(shader);
        return 0;
    }

    if (shader_out != NULL) {
        *shader_out = shader;
    }
    return 1;
}

static int screensave_gl33_link_program(
    screensave_gl33_state *state,
    const char *fragment_source,
    GLuint *program_out,
    GLint *viewport_location_out,
    GLint *texture_location_out
)
{
    GLuint vertex_shader;
    GLuint fragment_shader;
    GLuint program;
    GLint link_ok;
    char info_log[256];

    if (program_out != NULL) {
        *program_out = 0U;
    }
    if (viewport_location_out != NULL) {
        *viewport_location_out = -1;
    }
    if (texture_location_out != NULL) {
        *texture_location_out = -1;
    }

    if (
        state == NULL ||
        fragment_source == NULL ||
        !screensave_gl33_compile_shader(
            state,
            GL_VERTEX_SHADER,
            g_screensave_gl33_vertex_shader,
            &vertex_shader
        ) ||
        !screensave_gl33_compile_shader(
            state,
            GL_FRAGMENT_SHADER,
            fragment_source,
            &fragment_shader
        )
    ) {
        return 0;
    }

    program = state->create_program_fn();
    if (program == 0U) {
        state->delete_shader_fn(fragment_shader);
        state->delete_shader_fn(vertex_shader);
        screensave_gl33_emit_diag(
            state,
            SCREENSAVE_DIAG_LEVEL_ERROR,
            6983UL,
            "gl33_primitives",
            "glCreateProgram failed while preparing the GL33 shader pipeline."
        );
        return 0;
    }

    state->attach_shader_fn(program, vertex_shader);
    state->attach_shader_fn(program, fragment_shader);
    state->link_program_fn(program);
    link_ok = 0;
    state->get_program_iv_fn(program, GL_LINK_STATUS, &link_ok);
    if (link_ok == 0) {
        info_log[0] = '\0';
        state->get_program_info_log_fn(
            program,
            (GLsizei)sizeof(info_log),
            NULL,
            info_log
        );
        screensave_gl33_emit_diag(
            state,
            SCREENSAVE_DIAG_LEVEL_ERROR,
            6984UL,
            "gl33_primitives",
            info_log[0] != '\0'
                ? info_log
                : "The GL33 shader pipeline could not link a required program."
        );
        state->delete_program_fn(program);
        state->delete_shader_fn(fragment_shader);
        state->delete_shader_fn(vertex_shader);
        return 0;
    }

    state->delete_shader_fn(fragment_shader);
    state->delete_shader_fn(vertex_shader);

    if (viewport_location_out != NULL) {
        *viewport_location_out = state->get_uniform_location_fn(program, "u_viewport");
    }
    if (texture_location_out != NULL) {
        *texture_location_out = state->get_uniform_location_fn(program, "u_texture");
    }
    if (program_out != NULL) {
        *program_out = program;
    }
    return 1;
}

static int screensave_gl33_prepare_geometry(screensave_gl33_state *state)
{
    if (state == NULL) {
        return 0;
    }

    state->gen_vertex_arrays_fn(1, &state->vertex_array);
    state->gen_buffers_fn(1, &state->vertex_buffer);
    if (state->vertex_array == 0U || state->vertex_buffer == 0U) {
        screensave_gl33_emit_diag(
            state,
            SCREENSAVE_DIAG_LEVEL_ERROR,
            6985UL,
            "gl33_primitives",
            "The GL33 lane could not allocate its shared geometry objects."
        );
        return 0;
    }

    state->bind_vertex_array_fn(state->vertex_array);
    state->bind_buffer_fn(GL_ARRAY_BUFFER, state->vertex_buffer);
    state->buffer_data_fn(
        GL_ARRAY_BUFFER,
        (screensave_gl33_sizeiptr)(sizeof(screensave_gl33_vertex) * 12U),
        NULL,
        GL_STREAM_DRAW
    );
    state->enable_vertex_attrib_array_fn(0U);
    state->enable_vertex_attrib_array_fn(1U);
    state->enable_vertex_attrib_array_fn(2U);
    state->vertex_attrib_pointer_fn(
        0U,
        2,
        GL_FLOAT,
        GL_FALSE,
        (GLsizei)sizeof(screensave_gl33_vertex),
        (const GLvoid *)offsetof(screensave_gl33_vertex, x)
    );
    state->vertex_attrib_pointer_fn(
        1U,
        4,
        GL_UNSIGNED_BYTE,
        GL_TRUE,
        (GLsizei)sizeof(screensave_gl33_vertex),
        (const GLvoid *)offsetof(screensave_gl33_vertex, red)
    );
    state->vertex_attrib_pointer_fn(
        2U,
        2,
        GL_FLOAT,
        GL_FALSE,
        (GLsizei)sizeof(screensave_gl33_vertex),
        (const GLvoid *)offsetof(screensave_gl33_vertex, u)
    );
    state->bind_buffer_fn(GL_ARRAY_BUFFER, 0U);
    state->bind_vertex_array_fn(0U);
    return 1;
}

int screensave_gl33_pipeline_create(
    screensave_gl33_state *state,
    const char **failure_reason_out
)
{
    if (failure_reason_out != NULL) {
        *failure_reason_out = NULL;
    }

    if (state == NULL) {
        if (failure_reason_out != NULL) {
            *failure_reason_out = "gl33-pipeline-create-failed";
        }
        return 0;
    }

    if (
        !screensave_gl33_link_program(
            state,
            g_screensave_gl33_color_fragment_shader,
            &state->color_program,
            &state->color_viewport_location,
            NULL
        ) ||
        !screensave_gl33_link_program(
            state,
            g_screensave_gl33_texture_fragment_shader,
            &state->texture_program,
            &state->texture_viewport_location,
            &state->texture_sampler_location
        ) ||
        !screensave_gl33_prepare_geometry(state)
    ) {
        if (failure_reason_out != NULL) {
            *failure_reason_out = "gl33-pipeline-create-failed";
        }
        screensave_gl33_pipeline_destroy(state);
        return 0;
    }

    screensave_gl33_emit_diag(
        state,
        SCREENSAVE_DIAG_LEVEL_INFO,
        6986UL,
        "gl33_primitives",
        "The GL33 modern lane prepared its shared shader pipeline successfully."
    );
    return 1;
}

void screensave_gl33_pipeline_destroy(screensave_gl33_state *state)
{
    if (state == NULL) {
        return;
    }

    if (state->bind_vertex_array_fn != NULL) {
        state->bind_vertex_array_fn(0U);
    }
    if (state->bind_buffer_fn != NULL) {
        state->bind_buffer_fn(GL_ARRAY_BUFFER, 0U);
    }
    if (state->use_program_fn != NULL) {
        state->use_program_fn(0U);
    }
    if (state->delete_buffers_fn != NULL && state->vertex_buffer != 0U) {
        state->delete_buffers_fn(1, &state->vertex_buffer);
        state->vertex_buffer = 0U;
    }
    if (state->delete_vertex_arrays_fn != NULL && state->vertex_array != 0U) {
        state->delete_vertex_arrays_fn(1, &state->vertex_array);
        state->vertex_array = 0U;
    }
    if (state->delete_program_fn != NULL && state->color_program != 0U) {
        state->delete_program_fn(state->color_program);
        state->color_program = 0U;
    }
    if (state->delete_program_fn != NULL && state->texture_program != 0U) {
        state->delete_program_fn(state->texture_program);
        state->texture_program = 0U;
    }

    state->color_viewport_location = -1;
    state->texture_viewport_location = -1;
    state->texture_sampler_location = -1;
}

static int screensave_gl33_draw_vertices(
    screensave_gl33_state *state,
    GLuint program,
    GLint viewport_location,
    GLuint texture_id,
    const screensave_gl33_vertex *vertices,
    unsigned int vertex_count,
    GLenum mode
)
{
    if (
        state == NULL ||
        program == 0U ||
        vertices == NULL ||
        vertex_count == 0U
    ) {
        return 0;
    }

    state->use_program_fn(program);
    if (viewport_location >= 0) {
        state->uniform_2f_fn(
            viewport_location,
            (GLfloat)state->drawable_size.width,
            (GLfloat)state->drawable_size.height
        );
    }
    if (texture_id != 0U) {
        state->active_texture_fn(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, texture_id);
        if (state->texture_sampler_location >= 0) {
            state->uniform_1i_fn(state->texture_sampler_location, 0);
        }
    }

    state->bind_vertex_array_fn(state->vertex_array);
    state->bind_buffer_fn(GL_ARRAY_BUFFER, state->vertex_buffer);
    state->buffer_data_fn(
        GL_ARRAY_BUFFER,
        (screensave_gl33_sizeiptr)(sizeof(screensave_gl33_vertex) * vertex_count),
        vertices,
        GL_STREAM_DRAW
    );
    state->draw_arrays_fn(mode, 0, (GLsizei)vertex_count);
    state->bind_buffer_fn(GL_ARRAY_BUFFER, 0U);
    state->bind_vertex_array_fn(0U);
    if (texture_id != 0U) {
        glBindTexture(GL_TEXTURE_2D, 0U);
    }
    state->use_program_fn(0U);
    return 1;
}

static unsigned int screensave_gl33_bitmap_bytes_per_pixel(unsigned int bits_per_pixel)
{
    switch (bits_per_pixel) {
    case 24U:
        return 3U;
    case 32U:
        return 4U;
    default:
        return 0U;
    }
}

static void screensave_gl33_convert_bitmap_to_rgba(
    const screensave_bitmap_view *bitmap,
    unsigned char *pixels
)
{
    const unsigned char *source_bytes;
    unsigned int source_bytes_per_pixel;
    unsigned int y;
    unsigned int x;

    if (bitmap == NULL || pixels == NULL) {
        return;
    }

    source_bytes = (const unsigned char *)bitmap->pixels;
    source_bytes_per_pixel = screensave_gl33_bitmap_bytes_per_pixel(bitmap->bits_per_pixel);
    for (y = 0U; y < (unsigned int)bitmap->size.height; ++y) {
        const unsigned char *source_row;
        unsigned char *target_row;
        unsigned int source_y;

        source_y = bitmap->origin_top_left
            ? (unsigned int)(bitmap->size.height - 1) - y
            : y;
        source_row = source_bytes + ((size_t)source_y * (size_t)bitmap->stride_bytes);
        target_row = pixels + ((size_t)y * (size_t)bitmap->size.width * 4U);

        for (x = 0U; x < (unsigned int)bitmap->size.width; ++x) {
            const unsigned char *source_pixel;
            unsigned char *target_pixel;

            source_pixel = source_row + ((size_t)x * (size_t)source_bytes_per_pixel);
            target_pixel = target_row + ((size_t)x * 4U);
            target_pixel[0] = source_pixel[2];
            target_pixel[1] = source_pixel[1];
            target_pixel[2] = source_pixel[0];
            target_pixel[3] = source_bytes_per_pixel == 4U ? source_pixel[3] : 255U;
        }
    }
}

static int screensave_gl33_prepare_bitmap_texture(
    screensave_gl33_state *state,
    const screensave_bitmap_view *bitmap
)
{
    if (state == NULL || bitmap == NULL) {
        return 0;
    }

    if (state->bitmap_texture == 0U) {
        glGenTextures(1, &state->bitmap_texture);
        if (state->bitmap_texture == 0U) {
            screensave_gl33_emit_diag(
                state,
                SCREENSAVE_DIAG_LEVEL_WARNING,
                6988UL,
                "gl33_primitives",
                "glGenTextures failed for the cached GL33 bitmap blit texture."
            );
            return 0;
        }

        glBindTexture(GL_TEXTURE_2D, state->bitmap_texture);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    } else {
        glBindTexture(GL_TEXTURE_2D, state->bitmap_texture);
    }

    if (
        state->bitmap_texture_width != bitmap->size.width ||
        state->bitmap_texture_height != bitmap->size.height
    ) {
        glTexImage2D(
            GL_TEXTURE_2D,
            0,
            GL_RGBA,
            (GLsizei)bitmap->size.width,
            (GLsizei)bitmap->size.height,
            0,
            GL_RGBA,
            GL_UNSIGNED_BYTE,
            NULL
        );
        state->bitmap_texture_width = bitmap->size.width;
        state->bitmap_texture_height = bitmap->size.height;
    }

    return 1;
}

static unsigned char *screensave_gl33_prepare_bitmap_upload_buffer(
    screensave_gl33_state *state,
    const screensave_bitmap_view *bitmap
)
{
    size_t needed_bytes;
    unsigned char *new_pixels;

    if (state == NULL || bitmap == NULL) {
        return NULL;
    }

    needed_bytes = (size_t)bitmap->size.width * (size_t)bitmap->size.height * 4U;
    if (needed_bytes == 0U) {
        return NULL;
    }

    if (state->bitmap_upload_pixels == NULL || state->bitmap_upload_bytes < needed_bytes) {
        new_pixels = (unsigned char *)realloc(state->bitmap_upload_pixels, needed_bytes);
        if (new_pixels == NULL) {
            screensave_gl33_emit_diag(
                state,
                SCREENSAVE_DIAG_LEVEL_WARNING,
                6987UL,
                "gl33_primitives",
                "GL33 cached bitmap upload memory allocation failed."
            );
            return NULL;
        }
        state->bitmap_upload_pixels = new_pixels;
        state->bitmap_upload_bytes = needed_bytes;
    }

    return state->bitmap_upload_pixels;
}

void screensave_gl33_release_bitmap_cache(screensave_gl33_state *state)
{
    if (state == NULL) {
        return;
    }

    if (state->bitmap_texture != 0U) {
        glDeleteTextures(1, &state->bitmap_texture);
        state->bitmap_texture = 0U;
    }
    state->bitmap_texture_width = 0;
    state->bitmap_texture_height = 0;

    if (state->bitmap_upload_pixels != NULL) {
        free(state->bitmap_upload_pixels);
        state->bitmap_upload_pixels = NULL;
    }
    state->bitmap_upload_bytes = 0U;
}

void screensave_gl33_clear_impl(screensave_renderer *renderer, screensave_color color)
{
    screensave_gl33_state *state;

    if (!screensave_gl33_can_draw(renderer, &state)) {
        return;
    }

    glClearColor(
        (GLclampf)color.red / 255.0f,
        (GLclampf)color.green / 255.0f,
        (GLclampf)color.blue / 255.0f,
        (GLclampf)color.alpha / 255.0f
    );
    glClear(GL_COLOR_BUFFER_BIT);
}

void screensave_gl33_fill_rect_impl(
    screensave_renderer *renderer,
    const screensave_recti *rect,
    screensave_color color
)
{
    screensave_gl33_state *state;
    screensave_gl33_vertex vertices[6];
    GLfloat left;
    GLfloat top;
    GLfloat right;
    GLfloat bottom;

    if (!screensave_gl33_can_draw(renderer, &state) || rect == NULL) {
        return;
    }

    left = (GLfloat)rect->x;
    top = (GLfloat)rect->y;
    right = (GLfloat)(rect->x + rect->width);
    bottom = (GLfloat)(rect->y + rect->height);
    screensave_gl33_init_vertex(&vertices[0], left, top, color, 0.0f, 0.0f);
    screensave_gl33_init_vertex(&vertices[1], right, top, color, 0.0f, 0.0f);
    screensave_gl33_init_vertex(&vertices[2], right, bottom, color, 0.0f, 0.0f);
    screensave_gl33_init_vertex(&vertices[3], left, top, color, 0.0f, 0.0f);
    screensave_gl33_init_vertex(&vertices[4], right, bottom, color, 0.0f, 0.0f);
    screensave_gl33_init_vertex(&vertices[5], left, bottom, color, 0.0f, 0.0f);
    (void)screensave_gl33_draw_vertices(
        state,
        state->color_program,
        state->color_viewport_location,
        0U,
        vertices,
        6U,
        GL_TRIANGLES
    );
}

void screensave_gl33_draw_frame_rect_impl(
    screensave_renderer *renderer,
    const screensave_recti *rect,
    screensave_color color
)
{
    screensave_gl33_state *state;
    screensave_gl33_vertex vertices[4];
    GLfloat left;
    GLfloat top;
    GLfloat right;
    GLfloat bottom;

    if (!screensave_gl33_can_draw(renderer, &state) || rect == NULL) {
        return;
    }

    left = (GLfloat)rect->x;
    top = (GLfloat)rect->y;
    right = (GLfloat)(rect->x + rect->width);
    bottom = (GLfloat)(rect->y + rect->height);
    screensave_gl33_init_vertex(&vertices[0], left, top, color, 0.0f, 0.0f);
    screensave_gl33_init_vertex(&vertices[1], right, top, color, 0.0f, 0.0f);
    screensave_gl33_init_vertex(&vertices[2], right, bottom, color, 0.0f, 0.0f);
    screensave_gl33_init_vertex(&vertices[3], left, bottom, color, 0.0f, 0.0f);
    (void)screensave_gl33_draw_vertices(
        state,
        state->color_program,
        state->color_viewport_location,
        0U,
        vertices,
        4U,
        GL_LINE_LOOP
    );
}

void screensave_gl33_draw_line_impl(
    screensave_renderer *renderer,
    const screensave_pointi *start_point,
    const screensave_pointi *end_point,
    screensave_color color
)
{
    screensave_gl33_state *state;
    screensave_gl33_vertex vertices[2];

    if (
        !screensave_gl33_can_draw(renderer, &state) ||
        start_point == NULL ||
        end_point == NULL
    ) {
        return;
    }

    screensave_gl33_init_vertex(
        &vertices[0],
        (GLfloat)start_point->x,
        (GLfloat)start_point->y,
        color,
        0.0f,
        0.0f
    );
    screensave_gl33_init_vertex(
        &vertices[1],
        (GLfloat)end_point->x,
        (GLfloat)end_point->y,
        color,
        0.0f,
        0.0f
    );
    (void)screensave_gl33_draw_vertices(
        state,
        state->color_program,
        state->color_viewport_location,
        0U,
        vertices,
        2U,
        GL_LINES
    );
}

void screensave_gl33_draw_polyline_impl(
    screensave_renderer *renderer,
    const screensave_pointi *points,
    unsigned int point_count,
    screensave_color color
)
{
    screensave_gl33_state *state;
    screensave_gl33_vertex vertices[32];
    unsigned int index;

    if (
        !screensave_gl33_can_draw(renderer, &state) ||
        points == NULL ||
        point_count < 2U
    ) {
        return;
    }

    if (point_count > 32U) {
        point_count = 32U;
    }

    for (index = 0U; index < point_count; ++index) {
        screensave_gl33_init_vertex(
            &vertices[index],
            (GLfloat)points[index].x,
            (GLfloat)points[index].y,
            color,
            0.0f,
            0.0f
        );
    }

    (void)screensave_gl33_draw_vertices(
        state,
        state->color_program,
        state->color_viewport_location,
        0U,
        vertices,
        point_count,
        GL_LINE_STRIP
    );
}

int screensave_gl33_blit_bitmap_impl(
    screensave_renderer *renderer,
    const screensave_bitmap_view *bitmap,
    const screensave_recti *destination_rect
)
{
    screensave_gl33_state *state;
    unsigned char *upload_pixels;
    unsigned int source_bytes_per_pixel;
    GLenum upload_format;
    const void *upload_pixels_const;
    GLfloat left;
    GLfloat top;
    GLfloat right;
    GLfloat bottom;
    GLfloat top_v;
    GLfloat bottom_v;
    screensave_gl33_vertex vertices[6];
    screensave_color white;

    if (!screensave_gl33_can_draw(renderer, &state) || bitmap == NULL) {
        return 0;
    }

    source_bytes_per_pixel = screensave_gl33_bitmap_bytes_per_pixel(bitmap->bits_per_pixel);
    if (
        bitmap->pixels == NULL ||
        bitmap->size.width <= 0 ||
        bitmap->size.height <= 0 ||
        source_bytes_per_pixel == 0U
    ) {
        return 0;
    }

    if (!screensave_gl33_prepare_bitmap_texture(state, bitmap)) {
        return 0;
    }

    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    if (bitmap->bits_per_pixel == 32U) {
        upload_format = GL_BGRA;
        upload_pixels_const = bitmap->pixels;
        glPixelStorei(GL_UNPACK_ROW_LENGTH, bitmap->stride_bytes / 4);
    } else {
        upload_pixels = screensave_gl33_prepare_bitmap_upload_buffer(state, bitmap);
        if (upload_pixels == NULL) {
            glBindTexture(GL_TEXTURE_2D, 0U);
            return 0;
        }
        screensave_gl33_convert_bitmap_to_rgba(bitmap, upload_pixels);
        upload_format = GL_RGBA;
        upload_pixels_const = upload_pixels;
        glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);
    }
    glTexSubImage2D(
        GL_TEXTURE_2D,
        0,
        0,
        0,
        (GLsizei)bitmap->size.width,
        (GLsizei)bitmap->size.height,
        upload_format,
        GL_UNSIGNED_BYTE,
        upload_pixels_const
    );
    glPixelStorei(GL_UNPACK_ROW_LENGTH, 0);

    if (destination_rect != NULL) {
        left = (GLfloat)destination_rect->x;
        top = (GLfloat)destination_rect->y;
        right = (GLfloat)(destination_rect->x + destination_rect->width);
        bottom = (GLfloat)(destination_rect->y + destination_rect->height);
    } else {
        left = 0.0f;
        top = 0.0f;
        right = (GLfloat)bitmap->size.width;
        bottom = (GLfloat)bitmap->size.height;
    }
    top_v = bitmap->origin_top_left ? 0.0f : 1.0f;
    bottom_v = bitmap->origin_top_left ? 1.0f : 0.0f;

    white.red = 255U;
    white.green = 255U;
    white.blue = 255U;
    white.alpha = 255U;
    screensave_gl33_init_vertex(&vertices[0], left, top, white, 0.0f, top_v);
    screensave_gl33_init_vertex(&vertices[1], right, top, white, 1.0f, top_v);
    screensave_gl33_init_vertex(&vertices[2], right, bottom, white, 1.0f, bottom_v);
    screensave_gl33_init_vertex(&vertices[3], left, top, white, 0.0f, top_v);
    screensave_gl33_init_vertex(&vertices[4], right, bottom, white, 1.0f, bottom_v);
    screensave_gl33_init_vertex(&vertices[5], left, bottom, white, 0.0f, bottom_v);
    (void)screensave_gl33_draw_vertices(
        state,
        state->texture_program,
        state->texture_viewport_location,
        state->bitmap_texture,
        vertices,
        6U,
        GL_TRIANGLES
    );

    glBindTexture(GL_TEXTURE_2D, 0U);
    return 1;
}
