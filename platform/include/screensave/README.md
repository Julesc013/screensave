...
Current stage: the first narrow public header set remains real here, with Series 07 now using it for both the first real saver product and the first real diagnostics app:
...
- `renderer_api.h` for the renderer contract now exercised by the mandatory GDI backend and BenchLab's windowed harness path
