...
This is the default Windows saver MSVC lane for the frozen `C16` baseline plus the current substrate state.
...
The checked-in entry path for this lane is `build/msvc/vs2017_xp/ScreenSave.sln`.

This local lane targets the VS2017 `v141_xp` toolset for old-Windows smoke
testing and writes outputs under `out/msvc/vs2017_xp/`. Saver targets and the
shared platform library use the static MSVC runtime in this lane so the
generated `.scr` files are not coupled to a separately installed VS2017 runtime
redistributable during compatibility experiments.
When a VS2017 XP toolchain is available, the expected invocation shape is:

- open the solution in Visual Studio, or
- run `msbuild build\msvc\vs2017_xp\ScreenSave.sln /p:Configuration=Debug /p:Platform=Win32`

## Output Conventions

The projects write outputs under:

- `out/msvc/vs2017_xp/<Configuration>/<ProjectName>/`
- `out/intermediate/msvc/vs2017_xp/<Configuration>/<ProjectName>/`
